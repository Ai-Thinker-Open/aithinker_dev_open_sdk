...
static bool aiio_device_params_is_empty_check(aiio_xxj_fun_t *device_params)
{
    ...
    char switch_control_string[sizeof(device_params->switch_control)];
    char work_mode_string[sizeof(device_params->work_mode)];
    char spray_size_string[sizeof(device_params->spray_size)];
    memcpy(switch_control_string, &device_params->switch_control, sizeof(device_params->switch_control)); 
    memcpy(work_mode_string, &device_params->work_mode, sizeof(device_params->work_mode));
    memcpy(spray_size_string, &device_params->spray_size, sizeof(device_params->spray_size));
    if(aiio_buffer_is_empty_check(switch_control_string, sizeof(switch_control_string)))             return true;
    if(aiio_buffer_is_empty_check(work_mode_string, sizeof(work_mode_string)))             return true;
    if(aiio_buffer_is_empty_check(spray_size_string, sizeof(spray_size_string)))             return true;
    // if(device_params->switch_control == 0 || device_params->work_mode == 0 || device_params->spray_size == 0)
    // {
    //     return true;
    // }
...
int aiio_flash_get_device_params(aiio_xxj_fun_t *device_params)
{
    uint16_t data_len = 0;
    
    aiio_log_d("aiio_flash_read \r\n");
    data_len = sizeof(aiio_xxj_fun_t);
    aiio_flash_read(FLASH_RW_DEVICE_STATUS_ADDR, device_params, &data_len);

    aiio_log_d("aiio_xxj_fun_t_is_empty_check \r\n");
    if(aiio_device_params_is_empty_check(device_params))
    {
        aiio_log_e("Failed to get the data from the flash \r\n");
        return AIIO_ERROR;
    }

    aiio_log_d("device_params.switch_control = %d \r\n", device_params->switch_control);
    aiio_log_d("device_params.work_mode = %d \r\n", device_params->work_mode);
    aiio_log_d("device_params.spray_size = %d \r\n", device_params->spray_size);

    return data_len;
}

int aiio_flash_clear_device_params(aiio_xxj_fun_t *device_params)
{
    if(device_params == NULL)
    {
        aiio_log_e("param err \r\n");
    ...
    memset(device_params, 0, sizeof(aiio_xxj_fun_t));
    aiio_flash_erase(FLASH_RW_DEVICE_STATUS_ADDR, sizeof(aiio_xxj_fun_t));
    aiio_flash_write(FLASH_RW_DEVICE_STATUS_ADDR, device_params, sizeof(aiio_xxj_fun_t));
    aiio_log_d("device_params.switch_control = %d \r\n", device_params->switch_control);
    aiio_log_d("device_params.work_mode = %d \r\n", device_params->work_mode);
    aiio_log_d("device_params.spray_size = %d \r\n", device_params->spray_size);
...
int aiio_flash_save_device_params(aiio_xxj_fun_t *device_params)
{
    if(device_params == NULL)
    {
        aiio_log_e("param err \r\n");
