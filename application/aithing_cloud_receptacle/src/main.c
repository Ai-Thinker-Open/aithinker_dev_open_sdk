...
#define AIIO_UART0_DATABIT                           (8)
#define AIIO_UART0_STOPBIT                           (1)
#define AIIO_UART0_PARITY                            (0)
...
// 接收操作任务
static aiio_os_thread_handle_t *aiio_receptacle_oper_thread = NULL; /*!< handle of task thread*/
...
bool g_wifi_connect_u8 = false;           /*!< status of wifi connection*/
...
// 回调函数
static void aiio_wifi_event_cb(aiio_input_event_t *event, void *data)
{
    int32_t ret = 0;
    ...
    case AIIO_WIFI_EVENT_STA_DISCONNECTED:
    {
...
#ifdef CONFIG_BLE_DISTRIBUTION_NETWORK_ENABLE
        ...
        g_wifi_connect_u8 = false;
        wifi_start_connected = false;
#ifndef CONFIG_CLOUD_CONTROL_LED_MODULE
        aiio_cloud_receptacle_led_enable(true);
        aiio_cloud_receptacle_set_led_status(ONLY_GREEN_LED_TRIGGLE_STATUS);
        // aiio_turn_on_led_status(ONLY_GREEN_LED_TRIGGLE_STATUS);
#endif

#ifdef CONFIG_LAN_NETWORK_ENABLE
        aiio_user_lan_service_deinit();

#endif
        aiio_receptacle_report_status_clr();
    }
    break;
    ...
    case AIIO_WIFI_EVENT_STA_GOT_IP:
    {
        ...
        wifi_disconnect_count = 0;
        wifi_start_connected = false;
        aiio_receptacle_report_status_set();
    }
    break;
...
#ifdef CONFIG_AP_DISTRIBUTION_NETWORK_ENABLE
...
// 接收信号量处理
static void aiio_receptacle_oper_task(void *pvParameters)
{
    int msg_id;
    aiio_rev_queue_t rev_queue = {0};
    ...
    while (1)
    {
        ...
        if (xQueueReceive(cloud_rev_queue_handle, &rev_queue, 0) == pdPASS)
        {
            ...
            {
                aiio_log_e("REV_CONFIG_DATA_EVENT \r\n");
            ...
            // 通过从flash中的数据或BLE配置的数据来连接WIFI
            case REV_WIFI_CONNECTED_EVENT:
            {
                aiio_log_d("REV_WIFI_CONNECTED_EVENT \r\n");
                if (g_wifi_connect_u8 || wifi_start_connected)
                {
            ...
            case REV_CLOUD_DISCONNECTED:
            {
                ...
                if (!wifi_config_start && g_wifi_connect_u8)
                {
    ...
    aiio_os_thread_delete(aiio_receptacle_oper_thread);
}
...
// 串口定义初始化，用于AT指令的发送与接收
void aiio_receptacle_uart_init(void)
{
    ...
    uart_config.dataBits= AIIO_UART0_DATABIT;
    uart_config.stopBits= AIIO_UART0_STOPBIT;
    uart_config.parity= AIIO_UART0_PARITY;
    aiio_uart_deinit(uart_config);
    aiio_uart_init(uart_config);
}

// Flash初始化
int8_t aiio_receptacle_flash_init(void)
{
    aiio_flash_init();
...
// wifi 初始化
static int8_t aiio_receptacle_wifi_init(void)
{
    int32_t ret = 0;

    aiio_wifi_register_event_cb(aiio_wifi_event_cb);
...
// 定时器初始化
int8_t aiio_receptacle_timer_init(void)
{
    // 定时器初始化
    aiio_TimerInit();
...
// 外设初始化
int8_t aiio_receptacle_peripheral_init(void)
{
    aiio_rtc_time_init();
    // aiio_cloud_led_init();
    aiio_cloud_receptacle_led_init();
    aiio_cloud_key_init();
...
// 任务操作函数
int8_t aiio_receptacle_oper_init(void)
{
    ...
    aiio_os_thread_create(&aiio_receptacle_oper_thread, "aiio_receptacle_oper_task", aiio_receptacle_oper_task, 4096, NULL, 15);
    aiio_os_thread_create(&aiio_receptacle_report_thread, "aiio_receptacle_report_task", aiio_receptacle_report_task, 4096, NULL, 16);
    aiio_os_thread_create(&aiio_receptacle_power_thread, "aiio_receptacle_power_task", aiio_receptacle_power_task, 4096, NULL, 17);
    aiio_os_thread_create(&aiio_receptacle_voltage_sampling_thread, "aiio_receptacle_voltage_sampling_task", aiio_receptacle_voltage_sampling_task, 4096, NULL, 14);
    return 0;
}

// 获取WIFI配置信息
void aiio_receptacle_get_wificonfig(void)
{
    // 用于发送消息队列的变量
    aiio_rev_queue_t rev_queue = {0};
    ...
    // aiio_turn_on_led_status(ALL_LED_TRIGGLE_STATUS);
    aiio_cloud_receptacle_led_enable(true);
    aiio_cloud_receptacle_set_led_status(ONLY_BLUE_LED_TRIGGLE_STATUS);

    device_init = true;
}

// 系统初始化函数
static int8_t aiio_receptacle_system_init(void)
{
    ...
    // 串口初始化
    aiio_receptacle_uart_init();

    // Flash 初始化
    if (aiio_receptacle_flash_init() == -1)
    {
        return -1;
    }

    // WIFI 初始化
    if (aiio_receptacle_wifi_init() == -1)
    {
        return -1;
    }

    // 外设初始化
    aiio_receptacle_peripheral_init();

    // 定时器初始化
    if (aiio_receptacle_timer_init() == -1)
    {
        return -1;
    }

    // 操作任务
    if (aiio_receptacle_oper_init() == -1)
    {
        return -1;
    }

    // 获取WIFI配置信息
    aiio_receptacle_get_wificonfig();
...
void aiio_main(void *params)
{
    ...
    // 系统初始化
    if (aiio_receptacle_system_init() == -1)
    {
        aiio_log_e("aiio_receptacle_system_init status is minus\r\n");
    }

    while (1)
    {
        // 灯的状态处理，如果串口没有发送数据，那么500ms处理一次，连接成功为什么不显示，因为是把灯闪烁的状态关闭了
        led_status_tick++;
        if (led_status_tick > 25 && device_init)
        {
            aiio_cloud_receptacle_led_status_process();
            led_status_tick = 0;
        }

        if (control_led_count > 0)
        {
            led_control_tick++;
        }
        ...
        {
            aiio_log_e("################# led_control_tick = %d ###################\r\n", led_control_tick);
            aiio_log_e("################# control_led_count = %d ###################\r\n", control_led_count);
            control_led_count = 0;
            led_control_tick = 0;
        }
