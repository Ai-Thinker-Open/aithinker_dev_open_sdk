...
static aiio_os_thread_handle_t *aiio_udp_client_thread = NULL;
...
void aiio_udp_client_function(void)
{
    int client_socket;
    ...
    aiio_log_w("send to %s:%d message:%s\r\n",SERVER_IP,SERVER_PORT,message);
    if ((send_len = sendto(client_socket, message, strlen(message), 0, (struct sockaddr *)&server_addr, sizeof(server_addr))) == -1) {
        perror("sendto");
        close(client_socket);
        // exit(EXIT_FAILURE);
    }
    ...
    if ((recv_len = recvfrom(client_socket, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&server_addr, &server_addr_len)) == -1) {
        perror("recvfrom");
        close(client_socket);
        // exit(EXIT_FAILURE);
    }
    ...
    // 显示服务器发送的响应消息
    aiio_log_w("server send recive data:%.*s\n", recv_len, buffer);

    // 关闭客户端socket连接
    close(client_socket);

    aiio_os_thread_delete(aiio_udp_client_thread);
}
...
static void cb_wifi_event(aiio_input_event_t *event, void *data)
{
    ...
    case AIIO_WIFI_EVENT_STA_GOT_IP:
        ...
        if (aiio_os_thread_create(&aiio_udp_client_thread, "aiio_udp_client_thread", aiio_udp_client_function, 4096, NULL, __AIIO_OS_PRIORITY_DEFAULT) == AIIO_ERROR)
        {
            return;
        }
        break;
