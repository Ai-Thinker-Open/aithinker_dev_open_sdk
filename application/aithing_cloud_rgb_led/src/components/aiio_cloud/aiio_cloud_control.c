            ...
            if(cjson_value)
            {
                switch (cjson_value->type)
                {
                    case cJSON_Number:
                    {
                        ...
                        xSemaphoreGive(xSemaphore_light);
                    }
                    break;
...
#if 1//def CONFIG_CLOUD_CONTROL_LED_MODULE
...
static void aiio_receive_string_cmd(char *msgMid, char *from, uint8_t dpid, char *cmd)
{
#if 0//ndef CONFIG_CLOUD_CONTROL_LED_MODULE
    switch (dpid)
    {
        case CMD_PLAY_LIST:
        {
