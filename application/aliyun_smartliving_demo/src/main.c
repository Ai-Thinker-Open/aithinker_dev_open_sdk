...
// #define QCLOUD_IOT_MQTT_DIRECT_DOMAIN "mqtt://public.iot-as-mqtt.cn-shanghai.aliyuncs.com"
...
iotx_device_info_t DeviceInfo_info = {0};
...
void aiio_property_parse(void *data){
    ...
    if(cjson_params == NULL)
    {
     ...
     if (cJSON_IsObject(cjson_params)) {  
        cJSON *power_switch_item = cJSON_GetObjectItem(cjson_params, "powerstate");  
        if (power_switch_item != NULL) {    
            aiio_log_d("powerstate: %d\n", power_switch_item->valueint);  
...
void aiio_topic_recv_parse(char *topic, void *data){
    if (strstr(topic, "/thing/service/property/set") != NULL){
...
static aiio_err_t event_cb(aiio_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        snprintf(topic, MAX_SIZE_OF_CLOUD_TOPIC, "/sys/%s/%s/thing/service/property/set", DeviceInfo_info.product_key, DeviceInfo_info.device_name);
    ...
    while (1)
    {
        ...
        if(xQueueReceive(cloud_rev_queue_handle, &rev_queue, 0) == pdPASS)
        {
            switch (rev_queue.common_event)
            {
                ...
                {
                    /* Device AUTH */
                    if (0 != IOT_SetupConnInfo((void **)&pconn_info)) {
                        aiio_log_e("AUTH request failed!");
                        return;
                    }
                    mqtt_cfg.client_id = pconn_info->client_id;
                    mqtt_cfg.username = pconn_info->username;
                    mqtt_cfg.password = pconn_info->password;
      
                    mqtt_cfg.event_handle = event_cb;
                    mqtt_cfg.uri = wifi_config_data.region_mqtturl;   
                    mqtt_cfg.port = 1883;

                    aiio_log_i("[%s()-%d]MQTT connect config\r\n",__func__,__LINE__);
                    aiio_log_i("  host:%s:%d\r\n",mqtt_cfg.uri,mqtt_cfg.port);

                    aiio_log_i("@@@@ mqtt_cfg.client_id:%s\r\n",mqtt_cfg.client_id);
                    aiio_log_i("@@@@ mqtt_cfg.username:%s\r\n",mqtt_cfg.username);
                    aiio_log_i("@@@@ mqtt_cfg.password:%s\r\n",mqtt_cfg.password);
