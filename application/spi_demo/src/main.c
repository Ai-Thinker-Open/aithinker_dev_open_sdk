...
#define AIIO_SPI_PORT       CONFIG_SPI_PORT
...
void aiio_main(void *params)
{
    ...
    aiio_w25qxx_init(AIIO_SPI_PORT, &aiio_spi_pin_cfg);

    aiio_w25qxx_read_id(AIIO_SPI_PORT,rx_data,1000);

    aiio_log_w("25WQ16 ID: %x %x %x",rx_data[1],rx_data[2],rx_data[3]);
    
    aiio_w25qxx_read_status(AIIO_SPI_PORT,rx_data,1000);

    aiio_w25qxx_enable(AIIO_SPI_PORT,1000);

    aiio_w25qxx_read_status(AIIO_SPI_PORT,rx_data,1000);

    aiio_w25qxx_erase(AIIO_SPI_PORT,0x200,1000);

    while(1)
    {
        aiio_w25qxx_read_status(AIIO_SPI_PORT,status,1000);
    ...
    aiio_w25qxx_write(AIIO_SPI_PORT,0x200,tx_data,100,1000);
    ...
    aiio_w25qxx_read(AIIO_SPI_PORT,0x200,rx_data,100,1000);
