...
static bool aiio_wifi_config_data_is_empty_check(wifi_config_data_t *config_data)
{
    ...
    if (aiio_buffer_is_empty_check(config_data->ssid, sizeof(config_data->ssid)))
        return true;
    if (aiio_buffer_is_empty_check(config_data->passwd, sizeof(config_data->passwd)))
        return true;
    if (aiio_buffer_is_empty_check(config_data->wificc, sizeof(config_data->wificc)))
        return true;
    if (aiio_buffer_is_empty_check(config_data->mqttip, sizeof(config_data->mqttip)))
        return true;
    if (aiio_buffer_is_empty_check(config_data->token, sizeof(config_data->token)))
        return true;
    if (aiio_buffer_is_empty_check(config_data->tz, sizeof(config_data->tz)))
        return true;
    if (config_data->port == 0xff)
        return true;
...
static bool aiio_device_info_is_empty_check(aiio_device_info *device_info)
{
    ...
    if (aiio_buffer_is_empty_check(device_info->userName, sizeof(device_info->userName)))
        return true;
    if (aiio_buffer_is_empty_check(device_info->userPasswd, sizeof(device_info->userPasswd)))
        return true;
    if (aiio_buffer_is_empty_check(device_info->deviceId, sizeof(device_info->deviceId)))
        return true;
...
int aiio_flash_get_device_info(aiio_device_info *deviceInfo)
{
    char databuf[200] = {0};
    ...
    aiio_flash_read(FLASH_RW_DEVICE_INFO_ADDR, (uint8_t *)databuf, read_length);
    ...
    if(aiio_device_info_is_empty_check(deviceInfo))
    {
        aiio_log_e("Failed to get the data from the flash \r\n");
        return AIIO_ERROR;
    }

    aiio_log_d("deviceInfo.userName = %s \r\n", deviceInfo->userName);
    aiio_log_d("deviceInfo.userPasswd = %s \r\n", deviceInfo->userPasswd);
...
char *PacketDeviceInfoToJson(aiio_device_info *device_info)
{
...
int aiio_flash_save_device_info(aiio_device_info *device_info)
{
    ...
    deviceinfo = PacketDeviceInfoToJson(&device_info);
    aiio_log_d("deviceinfo = %s \r\n", deviceinfo);

    aiio_flash_erase(FLASH_RW_DEVICE_INFO_ADDR, strlen(deviceinfo) + 1);
    aiio_flash_write(FLASH_RW_DEVICE_INFO_ADDR, (uint8_t *)deviceinfo, strlen(deviceinfo));
...
static bool aiio_ota_info_is_empty_check(aiio_ota_msg_t *ota_info)
{
    ...
    if (aiio_buffer_is_empty_check(ota_info->otaVer, sizeof(ota_info->otaVer)))
        return true;
    if (aiio_buffer_is_empty_check(ota_info->otaType, sizeof(ota_info->otaType)))
        return true;
    if (aiio_buffer_is_empty_check(ota_info->download_url, sizeof(ota_info->download_url)))
        return true;
    if (aiio_buffer_is_empty_check(ota_info->PubId, sizeof(ota_info->PubId)))
        return true;
    if (aiio_buffer_is_empty_check(ota_info->md5, sizeof(ota_info->md5)))
        return true;
