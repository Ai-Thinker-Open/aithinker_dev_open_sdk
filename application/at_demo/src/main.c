...
#define FW_VER CONFIG_AT_FW_VER
#define SDK_VER CONFIG_AT_SDK_VER
void print_boot_info(void)
{
    char *chip_name;

    chip_name = aiio_get_chipname();
    aiio_at_printf("\r\n################################################\r\n");
    aiio_at_printf("\r\n");
...
void aiio_main(void *params)
{
    ...
    aiio_wdt_init_timeouts(1000*60);
    aiio_log_i("Combo AT!");
    ...
    aiio_at_run();
    if(at_basic_config.aiio_ate_flag){
        aiio_at_enable_echo();
    }else{
        aiio_at_disable_echo();
    }
#if CONFIG_AT_WIFI_CMD_ENABLE
