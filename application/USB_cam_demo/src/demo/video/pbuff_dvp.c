...
int picture_frame_push(pbuff_yuyv_frame_t *new_frame)
{
    ...
    if (new_frame->pbuff_id < 0) {
        aiio_log_i("ERROR: pbuff push id invalid\r\n");
        return -1;
    }
    ...
    if (ret == pdTRUE) {
        ...
        aiio_log_i("ERROR: pbuff push queue full\r\n");
        return -2;
    }
}
...
int picture_frame_free(pbuff_yuyv_frame_t *new_frame)
{
    ...
    if (new_frame->pbuff_id < 0) {
        aiio_log_i("ERROR: pbuff free id invalid");
        return -1;
    }
    ...
    if (ret == pdTRUE) {
        ...
        aiio_log_i("ERROR: pbuff free queue timeout\r\n");
        return -1;
    }
}
