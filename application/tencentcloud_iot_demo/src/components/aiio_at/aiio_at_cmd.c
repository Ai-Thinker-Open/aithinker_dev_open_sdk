...
void At_Key(char *cmd, uint16_t cmd_len)
{
    ...
    memset(&DeviceInfo_info, 0, sizeof(aiio_device_info));
    ...
    if(cmd_len > 0)
    {
        if (cmd[0] == '=')
        {
            ...
            memcpy(DeviceInfo_info.product_id, data, len);
            aiio_log_d("DeviceInfo_info.product_id = %s \r\n", DeviceInfo_info.product_id);
            ...
            len = strlen(temp);
            aiio_log_d("len = %d \r\n", len);
            memcpy(DeviceInfo_info.device_secret, temp, len);
            aiio_log_d("DeviceInfo.userPasswd = %s", DeviceInfo_info.device_secret);
