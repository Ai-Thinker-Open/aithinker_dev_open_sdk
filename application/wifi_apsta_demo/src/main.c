...
static void cb_wifi_event(aiio_input_event_t *event, void* data)
{
    switch (event->code) {
        ...
        case AIIO_WIFI_EVENT_AP_STACONNECTED:
            ...
            aiio_wifi_sta_basic_info_t sta_info;
            aiio_wifi_ap_sta_info_get(&sta_info, (uint8_t)event->value);
...
{
    int ret=aiio_wifi_ap_ip_set(EXAMPLE_AXK_WIFI_AP_IP);
    if(ret!=0){
        aiio_log_e("wifi ap_ip_set error!!");
    }

    aiio_wifi_set_mode(AIIO_WIFI_MODE_APSTA);
    ...
    strncpy((const char *)wifi_sta_config.sta.ssid, EXAMPLE_AXK_WIFI_STA_SSID, SSID_LEN - 1);
    strncpy((const char *)wifi_sta_config.sta.password, EXAMPLE_AXK_WIFI_STA_PASS, PASSWORD_LEN - 1);
