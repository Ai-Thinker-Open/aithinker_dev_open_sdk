            ...
            if (!aiio_http_client_is_chunked_response(evt->client)) {
                ...
                if (evt->user_data) {
                    ...
                    if (output_buffer == NULL) {
                        output_buffer = (char *) malloc(aiio_http_client_get_content_length(evt->client));
                        output_len = 0;
                        if (output_buffer == NULL) {
                            aiio_log_a( "Failed to allocate memory for output buffer");
                            return AIIO_FAIL;
                        }
                    }
...
static void http_rest_with_hostname_path(void)
{
    ...
    if (err == AIIO_OK) {
        aiio_log_a( "HTTP GET Status = %d, content_length = %lld",
                aiio_http_client_get_status_code(client),
                aiio_http_client_get_content_length(client));
    } else {
        aiio_log_a( "HTTP GET request failed: %s", "err");
    }

    // POST
    const char *post_data = "field1=value1&field2=value2";
    aiio_http_client_set_url(client, "/post");
    aiio_http_client_set_method(client, HTTP_METHOD_POST);
    aiio_http_client_set_post_field(client, post_data, strlen(post_data));
    err = aiio_http_client_perform(client);
    if (err == AIIO_OK) {
        aiio_log_a( "HTTP POST Status = %d, content_length = %lld",
                aiio_http_client_get_status_code(client),
                aiio_http_client_get_content_length(client));
    } else {
        aiio_log_a( "HTTP POST request failed: %s", "err");
    }

    //PUT
    aiio_http_client_set_url(client, "/put");
    aiio_http_client_set_method(client, HTTP_METHOD_PUT);
    err = aiio_http_client_perform(client);
    if (err == AIIO_OK) {
        aiio_log_a( "HTTP PUT Status = %d, content_length = %lld",
                aiio_http_client_get_status_code(client),
                aiio_http_client_get_content_length(client));
    } else {
        aiio_log_a( "HTTP PUT request failed: %s", "err");
    }

    //PATCH
    aiio_http_client_set_url(client, "/patch");
    aiio_http_client_set_method(client, HTTP_METHOD_PATCH);
    aiio_http_client_set_post_field(client, NULL, 0);
    err = aiio_http_client_perform(client);
    if (err == AIIO_OK) {
        aiio_log_a( "HTTP PATCH Status = %d, content_length = %lld",
                aiio_http_client_get_status_code(client),
                aiio_http_client_get_content_length(client));
    } else {
        aiio_log_a( "HTTP PATCH request failed: %s", "err");
    }

    //DELETE
    aiio_http_client_set_url(client, "/delete");
    aiio_http_client_set_method(client, HTTP_METHOD_DELETE);
    err = aiio_http_client_perform(client);
    if (err == AIIO_OK) {
        aiio_log_a( "HTTP DELETE Status = %d, content_length = %lld",
                aiio_http_client_get_status_code(client),
                aiio_http_client_get_content_length(client));
    } else {
        aiio_log_a( "HTTP DELETE request failed: %s", "err");
    }

    //HEAD
    aiio_http_client_set_url(client, "/get");
    aiio_http_client_set_method(client, HTTP_METHOD_HEAD);
    err = aiio_http_client_perform(client);
    if (err == AIIO_OK) {
        aiio_log_a( "HTTP HEAD Status = %d, content_length = %lld",
                aiio_http_client_get_status_code(client),
                aiio_http_client_get_content_length(client));
    } else {
        aiio_log_a( "HTTP HEAD request failed: %s", "err");
    }
       ...
       if(run_flag)
        {
                ...
                https_with_invalid_url();//https
                // https_with_hostname_path(); // http
        }
