...
int32_t aiio_blufi_ble_cb(aiio_blufi_cb_event_t event, aiio_blufi_cb_param_t *param)
{
    if(event == AIIO_BLUFI_EVENT_BLE_CONNECT){
        aiio_log_i("\r\n+EVENT:BLUFI_BLE_CONNECT\n");
    }else if(event == AIIO_BLUFI_EVENT_BLE_DISCONNECT){
        aiio_log_i("\r\n+EVENT:BLUFI_BLE_DISCONNECT\n");
    }
    return 0;
}
...
void aiio_main(void *params){
    ...
    blufi_cb.ble_event_cb =aiio_blufi_ble_cb;
