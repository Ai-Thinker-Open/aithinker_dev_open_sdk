...
static int init_ota_partition(void)
{
    ...
    if (ota_init_state == 0)
    {
        ...
        aiio_log_i("[OTA] activeID:%d \r\n", activeID);

        if (hal_boot2_get_active_entries(BOOT2_PARTITION_TYPE_FW, &otaEntry))
        {
            aiio_log_e("PtTable_Get_Active_Entries fail");
            return -1;
        }
        ...
        aiio_log_i("init_ota_partition success, ota_addr:0x%x, part_size:0x%x", ota_addr, part_size);
...
int partition_write_ota_farmware(int dst_offset, const void *src, int size)
{
    ...
    if (dst_offset - last_offset > 0x4000)
    {
        ...
        aiio_log_i("[OTA] write dst_offset:0x%0x", last_offset);
    }
...
int partition_erase(int start_addr, int size)
{
    ...
    {
        aiio_log_e("[OTA] erase out of area, part_size:0x%0x", part_size);
        return -1;
    }

    aiio_log_i("[OTA] erase start_addr:0x%0x size:0x%0x", start_addr, size);
...
void set_boot_partition(void)
{
    ...
    aiio_log_i("[OTA] ota size:0x%0x", fw_size);
