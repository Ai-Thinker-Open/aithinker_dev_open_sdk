...
/* dbi ili9488 config */
    ...
    /* Selecting interface type, more configuration of peripherals comes later
        ...
        2: simulation, supported functions: typeB-x8; (support chips: bl616),
    */
...
/* dpi gc9503v config */
    ...
    /* Selecting DPI working mode
        ...
        2: simulation (support: bl616, bl628)
    */
    ...
    */
    ...
    /* Selecting pixel format
        1: rgb565 (16-bits)
        2: nrgb8888 (32-bits, Do not support simulation)
    */
...
/********** simulation DBI configuration ***********/
#if (defined(LCD_DBI_INTERFACE_TYPE) && (LCD_DBI_INTERFACE_TYPE == 2))

    /* simulation mipi-dbi-typeB config, only 8-wire is supported, the read operation is not supported */

    /* dma used by sim */
    #define LCD_SIM_DBI_B_DMA_NAME "dma0_ch3"

    /* The maximum amount of data to be transferred affects the number of LLI memory pools */
    #define DBI_SIM_DBI_DATA_SIZE_MAX (800 * 640 * 4)

    /* Selecting pin */
    #define LCD_DBI_SIM_PIN_DC   GPIO_PIN_23
    #define LCD_DBI_SIM_PIN_WR   GPIO_PIN_34
    #define LCD_DBI_SIM_PIN_DATA GPIO_PIN_24 /* The lowest line of an 8-bit data line */
