...
#define CLOUD_RECONNECT_MAX_COUNT              (3)
...
static uint8_t cloud_reconnect_count = 0;                                      /*!< the number of cloud reconnect*/
        ...
        if(xQueueReceive(cloud_rev_queue_handle, &rev_queue, 0) == pdPASS)
        {
            switch (rev_queue.common_event)
            {
                case REV_CONFIG_DATA_EVENT:
                {
                    ...
                    AilinkSetWiFiCountryCode(wifi_config_data.wificc);
#ifdef CONFIG_AP_DISTRIBUTION_NETWORK_ENABLE
                    aiio_ap_config_stop();
#endif
                    rev_queue.common_event = REV_WIFI_CONNECTED_EVENT;
                ...
                case REV_CLOUD_CONNECTED_EVENT:
                {
                    ...
                    cloud_reconnect_count ++;
                    aiio_log_d("===== cloud_reconnect_count = %d \r\n",cloud_reconnect_count);
                    if(cloud_reconnect_count > CLOUD_RECONNECT_MAX_COUNT){
                        aiio_restart();
                    }
                ...
                case REV_CLOUD_ONLINE_EVENT:
                {
                    ...
                    cloud_reconnect_count = 0;   
                    cloud_connect_flag = true;
                    if(wifi_config_start)
                    {
...
void aiio_main(void *params){
    ...
    if(iotCheckDeviceInfoIsEmpty(&DeviceInfo) == 0)                   
    {
        ...
        snprintf(DataBuff, sizeof(DataBuff), "device info is empty\r\n");
        ...
        DEVICE_INFO_OUTPUT("############################## Device info  ##################################\r\n");
        snprintf(DataBuff, sizeof(DataBuff), "get device info is success\r\n");
        DEVICE_INFO_OUTPUT(DataBuff);
        memset(DataBuff, 0, sizeof(DataBuff));
        snprintf(DataBuff, sizeof(DataBuff), "build time: %s, build date: %s, fw version: %s \r\n", __TIME__, __DATE__,DEVICE_FW_VERSION);
        DEVICE_INFO_OUTPUT(DataBuff);
    ...
    while (1)
    {
        ...
        if(ota_transmit_init && (strncmp(OtaProInfo.otaType, IOT_OTA_TYPE_MCU, strlen(IOT_OTA_TYPE_MCU)) == 0 || strncmp(OtaProInfo.otaType, IOT_OTA_TYPE_EXTEND, strlen(IOT_OTA_TYPE_EXTEND))) == 0){
