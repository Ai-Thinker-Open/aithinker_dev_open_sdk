...
#define             STR_MD5                 "md5"
#define             STR_EXTENDS             "extends"
#define             STR_OTATYPE             "otatype"
#define             STR_KEY                 "key"
...
void McuPubliReportAttributes(void)
{
    ...
    while(aiio_ListGetNodeNum(&CmdDataList) > 0)
    {
        ...
        if(aiio_GetPacketReportAttributesType() == DEVICE_REPORT_TYPE){
            ret = aiio_service_report_attribute(aiio_GetMessageId(), AilinkGetProfrom(), json_str);
        }else{
...
uint8_t GetMcuOTAStart(void)
{
    return  McuOtaStart;
}


void ResetMcuOTAStart(void)
{
    McuOtaStart = false;
}
...
void NotifyMcuDeviceUpdate(char *otatype, char *ver, char *key, char *md5)
{
    char *info = NULL;

    if(ver == NULL || md5 == NULL || otatype == NULL)
    {
    ...
    snprintf(info, MCU_UPDATE_INFO_LEN, "{\"%s\":\"%s\", \"%s\":\"%s\", \"%s\":\"%s\", \"%s\":\"%s\"}", STR_OTATYPE, otatype, STR_VER, ver, STR_KEY, key, STR_MD5, md5);
...
void SetMcuOtaMode(uint8_t *mode, uint16_t len)
{
    if( mode[0] > XMODEM_1K)
    {
        aiio_log_e("Param fail\r\n");
    ...
    ota_transmit_mode = mode[0];
    aiio_log_i("ota_transmit_mode :%d",ota_transmit_mode);
    aiio_send_mcu_cmd(SET_MCU_OTA_MODEM_CMD, NULL, 0);
}
    ...
    switch (cmdType)
    {
        ...
        case WIFI_RESET_CMD:
        {
            ...
            if(mcuConnectFlag)
            {
                ...
                aiio_service_report_restore_report(NULL, NULL);
        ...
        break;
        case SET_MCU_OTA_MODEM_CMD:
        {
            aiio_log_d("SET_MCU_OTA_MODEM_CMD \r\n");

            len = ((DataCmd[LEN_H] << 8) | DataCmd[LEN_L]);
            aiio_log_d("len = %d \r\n", len);
            SetMcuOtaMode(&DataCmd[DATA_LOCAL], len);
...
int8_t UartProcessPro(void)
{
    ...
    if(ota_transmit_init && revLen>0 )
    {
        switch(processBuff[0]){
            case CRC16:
                    if(!McuOtaStart)
                    {
                    
                        aiio_log_i("McuOtaStart = %d \r\n", McuOtaStart);
                        McuOtaStart = true;
                    }
                break;
            case ACK:
                    McuOtaStatus = processBuff[0];
                break;
            case NAK:
                    McuOtaStatus = processBuff[0];
                break;
