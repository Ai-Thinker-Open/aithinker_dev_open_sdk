...
xmodem_mode_t    ota_transmit_mode = 0;
...
void AilinkSetWiFiCountryCode(char *wificc)
{
    if (wificc != NULL && strlen(wificc) > 0) {
        aiio_log_i("set country code: %s \r\n", wificc);
        aiio_wifi_set_country_code(wificc);
    }
}
...
static bool AilinkVerifyWifiInfoIsEmpty(wifi_config_data_t *info)
{
    ...
    if((strlen(info->ssid) == 0)  || (strlen(info->wificc) == 0) ||
...
void IotClearOTAMode(void)
{
    memset(&ota_transmit_mode, 0, sizeof(xmodem_mode_t));
    AilinkFlashWrite(DEVICE_OTA_MODE_FLASH_BLOCK, (uint8_t *)&ota_transmit_mode, sizeof(xmodem_mode_t));
}


void IotSaveOTAMode(void)
{
    AilinkFlashWrite(DEVICE_OTA_MODE_FLASH_BLOCK, (uint8_t *)&ota_transmit_mode, sizeof(xmodem_mode_t));
}


void IotLoadOTAMode(void)
{
    memset(&ota_transmit_mode, 0, sizeof(xmodem_mode_t));
    AilinkFlashRead(DEVICE_OTA_MODE_FLASH_BLOCK, (uint8_t *)&ota_transmit_mode, sizeof(xmodem_mode_t));
