...
int transp_enter(int id);
void transp_exit(int id);
...
static void transp_delete_tcpudp_link(int id)
{
    ...
    if (p->ServerSock > -1 && id == p->ServerId)
    {
...
static void cipserver_task_entry(void *params)
{
    ...
    //wifi_mode_t if_index = WIFI_MODE_AP;
    ...
    Log_i("One Client (%d) Connected", connected);
...
static int cip_server(int mode, int port)
{
    ...
    if (mode == 1)
    {
        Log_i("ServerId:%d  ServerSock:%d", p->ServerId, p->ServerSock);
        if (p->ServerSock != -1)
        {
