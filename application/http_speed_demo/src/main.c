...
static aiio_err_t _http_event_handler(aiio_http_client_event_t *evt)
{
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_FINISH:
            ...
            aiio_log_a("http GET speed is %d KB/s, timers:%d ms\r\n", (output_len>>10)*1000/speed_timer.cour_timer, speed_timer.cour_timer);
...
static void http_speed_url(void)
{
    uint32_t ip,gw,mask;
    aiio_wifi_sta_netif_get(&ip, &gw, &mask);
    sprintf(speed_timer.url_data, "http://%s:50010/speed_test.bin",ip4addr_ntoa(&gw));
    aiio_log_a("  IP: %s\r\n", ip4addr_ntoa(&ip));
    aiio_log_a("  GW: %s\r\n", ip4addr_ntoa(&gw));

    aiio_log_a("  url: %s\r\n", speed_timer.url_data);

    aiio_http_client_config_t config = {
            .url = &speed_timer.url_data[0],
