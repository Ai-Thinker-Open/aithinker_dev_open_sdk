...
bool wifi_connect_flag = false;                                           /*!< status of wifi connection*/
...
bool rd01_detection_state = false;
char packet_data[50] = "{\"header\":{\"ns\":\"iot.device.control\",\"name\":\"control\"},\"payload\":{\"control\":{\"1\":false}}}";
...
static void rd01_data_output_cb(uint8_t *data, uint16_t len)
{
    ...
    // }
    // printf("\r\n");
    // if(cloud_online_flag){
        // printf("rd01_detection_state is %d\r\n",rd01_detection_state);
        if(rd01_detection_state){
            // printf("11 data[8] is %02x\r\n",data[8]);
            if(data[8] == 0x00){
                // printf("00 data[8] is %02x\r\n",data[8]);
                strcpy(packet_data, "{\"header\":{\"ns\":\"iot.device.control\",\"name\":\"control\"},\"payload\":{\"control\":{\"1\":false}}}");
                // printf("00 %s\r\n",packet_data);
                aiio_mqtt_service_publish("PKEVvgUr/7MpYpDtA6eLLVN/control", packet_data, aiio_strlen(packet_data), 0, 0);
                aiio_green_led_status(false);
                rd01_detection_state = false;
                // printf("000 rd01_detection_state is %d\r\n",rd01_detection_state);
            } 
        }else{
        //    printf("11 data[8] is %02x\r\n",data[8]);
            if(data[8] != 0x00){
                strcpy(packet_data, "{\"header\":{\"ns\":\"iot.device.control\",\"name\":\"control\"},\"payload\":{\"control\":{\"1\":true}}}");
                // printf("11 %s\r\n",packet_data);
                aiio_mqtt_service_publish("PKEVvgUr/7MpYpDtA6eLLVN/control", packet_data, aiio_strlen(packet_data), 0, 0);
                aiio_green_led_status(true);
                rd01_detection_state = true;
                // printf("111 rd01_detection_state is %d\r\n",rd01_detection_state);
            }
        }
    // }
}
        ...
        if(xQueueReceive(cloud_rev_queue_handle, &rev_queue, 0) == pdPASS)
        {
            switch (rev_queue.common_event)
            {
                ...
                case REV_WIFI_CONNECTED_EVENT:
                {
                    ...
                    if(wifi_connect_flag || wifi_start_connected)
                    {
                ...
                case REV_CLOUD_DISCONNECTED:
                {
                    ...
                    if(!wifi_config_start && wifi_connect_flag)
                    {
