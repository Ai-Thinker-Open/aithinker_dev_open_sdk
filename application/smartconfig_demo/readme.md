...
$ ./build.sh bl618 smartconfig_demo menuconfig // 配置使用的平台IO信息以及log等级等

$ ./build.sh bl618 smartconfig_demo cn debug // 编译对应的平台demo
```

demo 简介
```
固件路径:aithinker_dev_sdk/out/bl618(对应平台命名)/smartconfig_demo/bin/dev_project.bin

配置信息：通过menuconfig的配置信息，见生成的"aiio_autoconf.h"文件内容，确认所选IO和其他配置信息
日志：通过串口调试助手查看输出日志信息，波特率：921600
外设硬件：详情联系深圳市安信可科技有限公司
功能：
smartconfig配网功能。需要手机开热点，热点名称必须为：myssid，密码：mypassword，频段在2.4G。需要下载“EspTouch”软件。
配网流程：模组连接热点->EspTouch随便连接一个wifi->EspTouch将连接到的wifi通过信道发送给模组->模组收到EspTouch发来的wifi并去连接该wifi。手动复位模组，通过串口调试助手查看log信息。
