...
static void aiio_ble_cb(aiio_ble_evt_t *event)
{
    switch (event->type) {
        ...
        case AIIO_BLE_EVT_DATA:
            aiio_log_i("data down handle:%d len:%d,DATA:%s", event->data.handle, event->data.length, event->data.data);
            break;
...
void aiio_main(void *params){
    ...
    aiio_os_thread_create(&aiio_test_ble_master_thread, "test_ble_master_thread", test_ble_master_thread, 4096, NULL, 1);
