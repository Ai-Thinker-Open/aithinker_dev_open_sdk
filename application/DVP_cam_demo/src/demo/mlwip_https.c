...
void mhttp_server_init()
{
    ...
    //    aiio_log_i(" write buf is: %d byte\n",snd_size);
    //    aiio_log_i(" 接收缓冲区原始大小为: %d 字节\n",snd_size);
    ...
    while (1)
    {
        aiio_log_i("accept start\r\n");
        sc = accept(ss, (struct sockaddr *)&client_addr, &addrlen);
        if ((sc < 0) || (mysemaphoreflag > 0))
        {
            aiio_log_i("accept fail sc is:%d  semaphore is:%d\r\n", sc, mysemaphoreflag);
            if (sc > 0)
            {
                close(sc);
            }
    ...
    while (1)
    {
        ...
        else if (strstr(command, "capture"))
        {
            ...
            if (ret < 0)
            {
                close(sc);
                mysemaphoreflag--;
                aiio_log_i("text write failed 3 1 %d", ret);
            }
            close(sc);
            mysemaphoreflag--;
...
void stream_task_process(void *msg)
{
    ...
    aiio_log_i("stream_task_process\r\n");
    ...
    while (1)
    {
        // aiio_log_i("stream_task_process while\r\n");
        ...
        if (ret < 0)
        {
            close(*pcb);
            mysemaphoreflag--;
            aiio_log_i("text write failed 1 %d", ret);
        ...
        if (ret < 0)
        {
            close(*pcb);
            mysemaphoreflag--;
            aiio_log_i("text write failed 2 %d", ret);
            xReturn = xSemaphoreGive(MuxSem_Handle); //给出互斥量
            // tls_gpio_write(WM_IO_PA_04, 0);
            vTaskDelete(NULL);
        }
        ...
        if (ret < 0)
        {
            close(*pcb);
            mysemaphoreflag--;
            aiio_log_i("text write failed 4 %d", ret);
            xReturn = xSemaphoreGive(MuxSem_Handle); //给出互斥量
            // tls_gpio_write(WM_IO_PA_04, 0);
            vTaskDelete(NULL);
        }
        if (streatask == 0)
        {
            aiio_log_i("text write failed 5 %d", ret);
            xReturn = xSemaphoreGive(MuxSem_Handle); //给出互斥量
            // tls_gpio_write(WM_IO_PA_04, 0);
            vTaskDelete(NULL);
        }
