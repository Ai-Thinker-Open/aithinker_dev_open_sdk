 ...
 */
     ...
     */
    int
    utils_aes_crypt_cbc(utils_aes_context *ctx,
                        int mode,
                        size_t length,
                        unsigned char iv[16],
                      ...
                      uint8_t mode, uint8_t *pKey, uint16_t keybits, uint8_t *iv)
    {
        ...
        if (UTILS_AES_ENCRYPT == mode)
        {
            ...
            if (ret != 0)
            {
                aiio_log_e("Set encry key err,ret:%d", ret);
                ret = AIIO_ERROR;
                goto exit;
            }
            ...
            // {
            //     aiio_log_e("Output buffer should not less than datalen+UTILS_AES_BLOCK_LEN for padding");
            //     ret = AIIO_ERROR;
            //     goto exit;
            // }
        ...
        else
        {
            ...
            if (ret != 0)
            {
                aiio_log_e("Set dencry key err,ret:%d", ret);
                ret = AIIO_ERROR;
                goto exit;
            }
        }
        ...
        if (ret != 0)
        {
            aiio_log_e("encryt err,ret:%d", ret);
            ret = AIIO_ERROR;
            goto exit;
        }
        else
