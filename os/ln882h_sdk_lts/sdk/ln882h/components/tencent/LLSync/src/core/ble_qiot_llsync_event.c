...
ble_qiot_ret_status_t ble_event_report_device_info(uint8_t type)
{
    ...
    mtu_size |= ble_get_user_data_mtu_size((e_system)ble_system_type_get());
                                        ...
                                        const char *buf, uint16_t buf_len)
{
    ...
    do {
        ...
        ble_qiot_log_hex(BLE_QIOT_LOG_LEVEL_INFO, "post data(LN882H->Wechat)", (char *)send_buf, send_len);
