...
static void (*sg_ble_qiot_dev_start_cb)(void) = NULL;
...
int ble_device_info_msg_handle(const char *in_buf, int in_len)
{
    ...
    ble_qiot_log_hex(BLE_QIOT_LOG_LEVEL_INFO, "tlv(WeChat->LN882H)", p_data, p_data_len);
    ...
    switch (ch) {
        ...
        case E_DEV_MSG_SET_WIFI_MODE:
            ret = ble_combo_wifi_mode_set((BLE_WIFI_MODE)p_data[1]);
            break;
