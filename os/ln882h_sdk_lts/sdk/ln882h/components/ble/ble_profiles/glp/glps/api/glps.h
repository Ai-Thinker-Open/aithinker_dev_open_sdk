...
#define GLPS_IS(IDX, FLAG) ((p_glps_env->p_env[IDX]->flags & (1<< (GLPS_##FLAG))) == (1<< (GLPS_##FLAG)))

/// Set flag enable
#define GLPS_SET(IDX, FLAG) (p_glps_env->p_env[IDX]->flags |= (1<< (GLPS_##FLAG)))

/// Set flag disable
#define GLPS_CLEAR(IDX, FLAG) (p_glps_env->p_env[IDX]->flags &= ~(1<< (GLPS_##FLAG)))
...
struct glps_env_tag
{
    ...
    struct glps_cnx_env *p_env[GLPS_IDX_MAX];
 ...
 *
 * @param[out] p_packed_meas    Glucose measurement value packed
 * @param[in] p_meas_val        Glucose measurement value
 * @param[in] seq_num           Glucose measurement sequence number
 * @param[in] p_glps_env        Glucose Profile service environment variable
 *
 ...
 *
 * @param[out] p_packed_meas_ctx    Glucose measurement context value packed
 * @param[in] p_meas_ctx_val        Glucose measurement context value
 * @param[in] seq_num               Glucose measurement sequence number
 *
 * @return size of packed value
 ****************************************************************************************
 ...
 * @param[in] length packed value length
 * @param[out] p_racp_req Record Access Control Request value
 *
 ...
 * @param[in] p_racp_rsp Record Access Control Response value
 *
