...
struct anpc_env_tag
{
    ...
    struct anpc_cnx_env *p_env[ANPC_IDX_MAX];
 ...
 * @param[in] p_anpc_env Environment variable
...
/**
 ****************************************************************************************
 * @brief Try to find the next Alert Category
 * @param[in] p_anpc_env Environment variable
 * @param[in] conidx Connection index
 * @param[in] cat_id Alert category bit mask
 * @return If next alert category was found or not.
 *****************************************************************************************
 */
bool anpc_found_next_alert_cat(struct anpc_env_tag *p_anpc_env, uint8_t conidx,
        struct anp_cat_id_bit_mask cat_id);

/**
 ****************************************************************************************
 * @brief Write over GATT to enable sending of notifications for the found category ID
 * @param[in] p_anpc_env Environment variable
 * @param[in] conidx Connection index
 * @param[in] cmd_id Command ID @see enum anp_cmd_id
 * @param[in] cat_id Alert category bit mask
 *****************************************************************************************
 */
void anpc_write_alert_ntf_ctnl_pt(struct anpc_env_tag *p_anpc_env, uint8_t conidx,
