...
#define CPPS_IDX(hdl) \
    ((hdl - p_cpps_env->shdl) + \
        ((!(CPPS_IS_FEATURE_SUPPORTED(p_cpps_env->prfl_cfg, CPPS_MEAS_BCST_MASK)) && \
                ((hdl - p_cpps_env->shdl) > CPS_IDX_CP_MEAS_BCST_CFG)) ? (1) : (0)) + \
        ((!(CPPS_IS_FEATURE_SUPPORTED(p_cpps_env->prfl_cfg, CPPS_VECTOR_MASK)) && \
                ((hdl - p_cpps_env->shdl) > CPS_IDX_VECTOR_CHAR)) ? (3) : (0)))
 ...
 * @param[out] p_pckd_meas pointer to packed message
 ...
 * @param[out] p_meas_val message to split
 * @return length of the second notification
 ****************************************************************************************
 */
uint8_t cpps_split_meas_ntf(uint8_t conidx, struct gattc_send_evt_cmd *p_meas_val);
 ...
 * @param[in] p_param Pointer to the parameters of the message.
 * @param[out] p_pckd_vector pointer to packed message
 * @return length
 ****************************************************************************************
 */
 ...
 ****************************************************************************************
 */
void cpps_task_init(struct ke_task_desc *p_task_desc);
