...
void spi_flash_read_id(uint8_t *id)
{
    ...
    uint8_t buffer[4];
    spi_master_write_and_read_data(CMD_READ_ID,buffer,4);
...
void spi_flash_read_status(uint8_t *status)
{
    ...
    spi_master_write_and_read_data(CMD_READ_STATUS_1,buffer,2);
    memcpy(status,buffer+1,1);
    spi_master_write_and_read_data(CMD_READ_STATUS_2,buffer,2);
...
void spi_flash_write_enable()
{
    ...
    uint8_t buffer[1];
    spi_master_write_and_read_data(CMD_WRITE_ENABLE,buffer,1);
}
...
void spi_flash_erase_flash(uint32_t add)
{
    ...
    spi_master_write_and_read_data(CMD_ERASE_FLASH,buffer,4);
}
...
{   
    ...
    spi_master_write_data_with_addr(buffer,4,data,length);
}
...
void spi_flash_read_flash(uint32_t addr ,uint8_t *data, uint32_t length)
{
    ...
    spi_master_read_data_with_addr(buffer,4,data,length);
}
