 ...
 * - @b net_if_t: Network stack internal representation of a interface. FHOST module simply
 *                keep a reference to such structure it is never dereferenced and as such
 *                doesn't need to be fully defined.
 * - @b net_buf_rx_t: Network stack header for a RX buffer (i.e. doesn't contains data,
 *                    just a pointer to it). It is allocated (but not initialized) by the
 *                    FHOST module and must then be fully defined.
 * - @b net_buf_tx_t: Network stack header for a TX buffer (i.e. doesn't contains data,
