...
// FHOST functions to be called by net_al
extern void fhost_task_ready(enum rtos_task_id task_id);
extern int fhost_tx_start(net_if_t *net_if, net_buf_tx_t *net_buf,
               ...
               void *vif)
{
    ...
    *net_if = (net_if_t *)malloc(sizeof(net_if_t));
    memset(*net_if, 0, sizeof(net_if_t));
...
void net_if_up_cb(net_if_t *net_if)
{
    netifapi_netif_set_up(net_if);
}

void net_if_down_cb(net_if_t *net_if)
{
...
int net_l2_send_do(net_if_t *net_if, const uint8_t *data, int data_len, uint16_t ethertype,
                   const uint8_t *dst_addr, bool *ack)
{
    ...
    res = fhost_tx_start(net_if, pbuf, net_l2_send_cfm, ack);
