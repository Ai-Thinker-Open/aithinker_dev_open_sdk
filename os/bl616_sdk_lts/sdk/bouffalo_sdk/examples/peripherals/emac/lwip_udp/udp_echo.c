...
#define UDP_TEST_SELECT UDP_SOCKET_TEST
...
static void udp_echo_netconn_test(void)
{
    ...
    if (err != ERR_OK) {
        printf("UDP netconn bind failed\r\n");
        return;
    }
    ...
    while (1) {
        ...
        if (err == ERR_OK) {
            if (netbuf_copy(buf, recv_buf, sizeof(recv_buf)) != buf->p->tot_len) {
                printf("netconn copy failed\r\n");
            } else {
                recv_buf[buf->p->tot_len] = '\0';
                err = netconn_send(conn, buf);
                if (err != ERR_OK) {
                    printf("UDP netconn send failed\r\n");
                }
                // else {
                // printf("UDP get:%s\r\n", recv_buf);
                // }
...
static void udp_echo_socket_test(void)
{
    ...
    while (1) {
        ...
        printf("udp socket creat success\r\n");
...
void udp_echo_raw_init(void)
{
    ...
    if (udp_echo_raw_pcb != NULL) {
        ...
        if (err == ERR_OK) {
            // printf("bind UDP PCB:0x%x!\r\n", udp_echo_raw_pcb);
            udp_recv(udp_echo_raw_pcb, udp_echo_raw_recv, NULL);
        } else {
            printf("UDP bind error,err:%d\r\n", err);
        }
    } else {
        printf("Creat UDP falied!\r\n");
    }
}
...
void udp_task_test(void)
{
    ...
    if (udp_echo_raw_pcb != NULL) {
        ...
        if (err != ERR_OK) {
            printf("UDP connect error,err:%d\r\n", err);
            return;
        }
