...
struct __attribute__((packed, aligned(4))) sys_clk_cfg_t {
    ...
    uint8_t wlp2_mcu_clk;
    uint8_t wlp2_mcu_clk_div;

    uint8_t wlp2_mcu_bclk_div;
    uint8_t wlp1_root_clk;
    uint8_t wlp1_root_bclk_div;
...
struct __attribute__((packed, aligned(4))) boot_basic_cfg_t {
    ...
    uint32_t rsvd_2             : 4; /* [21: 18] rsvd_2 */
...
struct __attribute__((packed, aligned(4))) bootheader_t {
    ...
    struct boot_cpu_cfg_t cpu_cfg[2]; /* 16 */
    ...
    uint32_t rsvd0[4]; /* rsvd */
    uint32_t rsvd1[4]; /* rsvd */

    uint32_t crc32; /* 4 */
};

#define BFLB_FW_LENGTH_OFFSET 136
#define BFLB_FW_HASH_OFFSET   140
