                    ...
                    uint32_t pksize, uint32_t duration, uint16_t tos)
{
    ...
    if (stream == NULL)
    {
        fhost_print(RTOS_TASK_NULL, "Invalid stream id");
        return 1;
    }

    if (stream->active)
    {
        fhost_print(RTOS_TASK_NULL, "Stream %d is already active", stream_id);
        return 1;
    }

    if ((direction != FHOST_TG_DIRECT_SEND) && (direction != FHOST_TG_DIRECT_RECV))
    {
        fhost_print(RTOS_TASK_NULL, "Invalid direction");
        return 1;
    }

    if (prof >= FHOST_PROF_LAST)
    {
        fhost_print(RTOS_TASK_NULL, "Invalid profile");
        return 1;
    }
...
int fhost_tg_start(uint32_t stream_id)
{
    ...
    else if (stream->prof.direction == FHOST_TG_DIRECT_RECV)
    {
        switch (stream->prof.prof_id)
        {
            ...
            case FHOST_PROF_TRANSC :
                ...
                break;
            default :
                fhost_print(RTOS_TASK_NULL, "Invalid profile for RX");
                return 1;
        }
        stream->active = true;
        return 0;
    ...
    {
        fhost_print(RTOS_TASK_NULL, "Invalid stream direction");
        return 1;
    }
}
...
struct fhost_tg_stats *fhost_tg_stop(uint32_t stream_id)
{
    ...
    if (!stream->active)
    {
        fhost_print(RTOS_TASK_NULL, "Stream %d is not active", stream_id);
        return NULL;
    }
