...
void int_array_sort_unique(int *a)
{
	...
	wpa_selection_sort(a, alen, sizeof(int), freq_cmp);
#else
...
#ifdef CONFIG_EXTRA_SIZE_OPT
static inline void swap_(void *a, void *b, size_t size)
{
    char *a_ = (char *)a, *b_ = (char *)b, t;
    while (size-- > 0) {
        t = *a_;
        *a_++ = *b_;
        *b_++ = t;
...
void wpa_selection_sort(void *base, size_t nmemb, size_t size, int (*compare)(const void *, const void *))
{
    ...
    for (i = 0; i < nmemb - 1; i++) {
        m_i = i;
        for (j = i + 1; j < nmemb; j++)
            if (compare(base + j * size, base + m_i * size) < 0)
                m_i = j;
        if (m_i != i)
            swap_(base + i * size, base + m_i * size, size);
    }
}
#endif
