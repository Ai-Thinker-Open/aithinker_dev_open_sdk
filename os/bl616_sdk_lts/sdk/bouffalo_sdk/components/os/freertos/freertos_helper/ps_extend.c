...
#if !configGENERATE_RUN_TIME_STATS || !INCLUDE_xTaskGetIdleTaskHandle
#error please enable configGENERATE_RUN_TIME_STATS and INCLUDE_xTaskGetIdleTaskHandle
#endif

static void dummy_config_check()
{
    void DUMMY_PLEASE_set_configRUN_TIME_COUNTER_TYPE_uint64_t(void);
    if(sizeof(configRUN_TIME_COUNTER_TYPE) != 8)
        DUMMY_PLEASE_set_configRUN_TIME_COUNTER_TYPE_uint64_t();
}
