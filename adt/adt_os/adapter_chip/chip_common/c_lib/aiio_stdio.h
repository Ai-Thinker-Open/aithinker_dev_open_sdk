...
CHIP_API size_t aiio_strnlen(const char *s, size_t maxlen);

CHIP_API int aiio_strcmp (const char *p1, const char *p2);

CHIP_API int aiio_atoi(char* str);

CHIP_API char *aiio_strdup(const char *src);

CHIP_API char *aiio_strndup(const char *src, size_t n);

CHIP_API int aiio_vasprintf(char **ret, const char *format, va_list ap);

CHIP_API int aiio_asprintf(char **ret, const char *format, ...);
