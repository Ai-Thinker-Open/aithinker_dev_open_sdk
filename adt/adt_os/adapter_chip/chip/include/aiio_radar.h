...
typedef void(*rd01_cmd_ack_t)(uint8_t *data, uint16_t len);
...
/** @brief Rd-01 cmd ack callback register
 *
 *  @param[in]      cb              Callback function
 *  @return                         Return the operation status. When the return value is AIIO_OK, is successful.
 *  @retval         AIIO_OK         Init successful.
 ...
 *  @see
 */
CHIP_API int32_t aiio_rd01_cmd_ack_cb_register(rd01_cmd_ack_t cb);

/** @brief Rd-01 cmd process function
 *
 *  @param[in]      buf             Buff
 ...
 *  @see
 */
CHIP_API void aiio_radar_cmd_process(uint8_t* buf, uint16_t len);
