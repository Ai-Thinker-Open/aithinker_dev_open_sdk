...
#define     AIIO_GPIO_C             __AIIO_GPIO_C
...
typedef enum
{
    ...
    AIIO_GPIO_PULL_UP = 1,
    AIIO_GPIO_PULL_NONE = 2,
} aiio_gpio_pull_t;
/**
 * @brief IO Interrupt callback functions
 */
typedef void(*aiio_hal_gpio_int_cb_t)(int32_t irq,void *param);
 ...
 */
CHIP_API int32_t aiio_hal_gpio_pin_direction_set(uint32_t port, uint16_t pin, aiio_gpio_direction_t dir);
 ...
 * @return Status  - 0: Succeed, else: fail
 */
CHIP_API int32_t aiio_hal_gpio_pin_pull_set(uint32_t port, uint16_t pin, aiio_gpio_pull_t pull);
...
/**
 * @brief API to get hal gpio by map,this just use in combo at cmd platform
 * @param[in]  at_pin The IO pin in at map
 * @param[out]  p_port The aiio hal gpio port
 * @param[out]  p_pin The aiio hal gpio pin
 * @return Successfully returned AIIO_SUCC, Other failed. @see AIIO_ERR_E
 * @example
 *     uint32_t hal_port=AIIO_GPIO_A;
 *     uint16_t hal_pin=AIIO_GPIO_NUM_0;
 *     if(AIIO_OK!=aiio_at_get_gpio_by_map(aiio_iomap_save_config.ioMap[modePin],&hal_port,&hal_pin)){//WAIT_TODO
 *         aiio_log_e("at_pin=%d aiio_at_get_gpio_by_map() error",aiio_iomap_save_config.ioMap[modePin]);
 *         return -1;
 *     }
 */
CHIP_API aiio_ret_t aiio_at_get_gpio_by_map(uint8_t at_pin,uint32_t *p_port, uint16_t *p_pin);
