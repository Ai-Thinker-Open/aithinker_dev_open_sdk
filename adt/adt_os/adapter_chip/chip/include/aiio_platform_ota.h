 ...
 */
CHIP_API int aiio_platform_ota_start(void);
 ...
 * @return  int  Reture all kind of error code that is defined in aiio_error.h
 */
CHIP_API int aiio_platform_ota_stop(void);
 ...
 */
CHIP_API int aiio_platform_ota_install(uint8_t *data, uint32_t data_len, uint32_t data_total_len);
...
/**
 * @brief   Obtain a safe Flash address for temporary storage of MCU OTA data
 *
 * @param[in]   need_size   The required cache space size
 * @param[out]  out_addr    The safe starting address
 *
 * @return  int  AIIO_OK means out_addr is valid, otherwise it is the error code defined in aiio_error.h
 */
CHIP_API int aiio_platform_get_ota_cache_addr(uint32_t need_size, uint32_t *out_addr);
