...
typedef enum {
    AIIO_BLE_CCC_NONE = 0x00,
    AIIO_BLE_CCC_NOTIFY,
    AIIO_BLE_CCC_INDICATE,
} aiio_ble_ccc_type_t;
...
#define AIIO_BLE_PAIRING_MODE_NO_PAIRING   0x00
#define AIIO_BLE_PAIRING_MODE_PAIRABLE     0x01

typedef struct {
    /* AIIO_BLE_PAIRING_MODE */
    uint8_t pairmode;
    uint32_t passkey;
} aiio_ble_smp_param_t;

typedef struct {
    aiio_ble_conn_interval_t conn_intv;
    aiio_ble_smp_param_t smp_cfg;
} aiio_ble_config_t;
...
/** @brief Register BLUFI default service
 *
 *  @param[in]      cfg             Default gatt server uuid config
 *  @param[out]     handle          Operate handle
 *  @return                         Return the operation status. When the return value is AIIO_OK, is successful.
 ...
 *  @see
 */
CHIP_API aiio_err_t ble_gatts_add_blufi_svcs(aiio_ble_default_server_t *cfg, aiio_ble_default_handle_t *handle);

/** @brief CCC config
 *
 *  @param[in]      cfg             Default gatt cccconfig
 *  @return                         Return the operation status. When the return value is AIIO_OK, is successful.
 *  @retval         AIIO_OK         Init successful.
 *  @retval         AIIO_ERROR      Init error.
 *  @note
 *  @see
 */
CHIP_API aiio_err_t ble_gattc_ccc_cfg(const aiio_ble_ccc_t *cfg);

/** @brief set conn tx powe
 *
 *  @param[in]      conn            Connect handle
 ...
 *  @see
 */
CHIP_API aiio_err_t aiio_ble_set_conn_power(aiio_ble_conn_t conn, int8_t power);

/** @brief set adv tx powe
 *
 *  @param[in]      power           tx power
 *  @return                         Return the operation status. When the return value is AIIO_OK, is successful.
 *  @retval         AIIO_OK         Init successful.
 ...
 *  @see
 */
CHIP_API aiio_err_t aiio_ble_set_adv_power(int8_t power);
#endif
