...
CHIP_API int32_t aiio_os_thread_start_scheduler(void);

CHIP_API uint32_t aiio_os_ms2tick(uint32_t ms);
CHIP_API uint32_t aiio_os_tick2ms(uint32_t tick);

CHIP_API uint32_t aiio_os_get_free_heap_size(void);
CHIP_API uint32_t aiio_os_get_minimum_ever_free_heap_size(void);
