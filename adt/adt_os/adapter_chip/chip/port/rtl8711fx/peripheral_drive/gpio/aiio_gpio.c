...
{
    /* On RTL8721F the ROM function GPIO_DeInit() appears to write the
     * port-wide GPIO_DDR register non-atomically, which silently resets
     * the OUTPUT direction of other pins on the same port (e.g. PA19
     * loses OUTPUT when PA20 is deinited).  Skipping the ROM call here
     * is safe because gpio_init() called immediately afterwards fully
     * reinitialises the pin anyway. */
    (void)port;
    (void)pin;
    return AIIO_OK;
}
