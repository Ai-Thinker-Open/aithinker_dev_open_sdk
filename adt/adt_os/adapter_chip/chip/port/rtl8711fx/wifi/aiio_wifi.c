...
static void aiio_wifi_client_connected_cb(u8 *evt_info);
static void aiio_wifi_client_disconnected_cb(u8 *evt_info);
static aiio_wifi_beacon_auth_t rtl_security2aiio(enum rtw_security rtl_security_val);
static void aiio_wifi_join_status_cb(u8 *evt_info);
...
extern struct rtw_event_hdl_func_t event_external_hdl[3];
extern u16 array_len_of_event_external_hdl;

struct rtw_event_hdl_func_t event_external_hdl[3] = {
	{RTW_EVENT_JOIN_STATUS,			aiio_wifi_join_status_cb},
    {RTW_EVENT_AP_STA_ASSOC,		aiio_wifi_client_connected_cb},
    {RTW_EVENT_AP_STA_DISASSOC,		aiio_wifi_client_disconnected_cb},
};
...
// #endif
    ...
    // AP模式下连接、断开回调
    // wifi_reg_event_handler(RTW_EVENT_AP_STA_ASSOC, aiio_wifi_client_connected_cb, NULL);
    // wifi_reg_event_handler(RTW_EVENT_AP_STA_DISASSOC, aiio_wifi_client_disconnected_cb, NULL);
...
aiio_err_t aiio_wifi_rssi_get(int32_t *rssi)
{
    ...
    *rssi= (signed char)(0xFF - phy_stats.sta.rssi + 1);
...
static aiio_os_function_return_t rtl_wifi_connect_default(void){
    ...
    //Start DHCPClient
    aiio_log_i("LwIP_DHCP start");
	LwIP_DHCP(0, DHCP_START);
    aiio_log_i("LwIP_DHCP end");
...
static void aiio_wifi_client_connected_cb(u8 *evt_info)
{
    ...
    // buf[10]~buf[15]是新加入设备的MAC地址
    uint8_t *mac = (uint8_t *)join_status_info->bssid;
    uint8_t idx = 0;
...
static void aiio_wifi_join_status_cb(u8 *evt_info)
{
	...
	struct rtw_event_join_status_info *join_info = (struct rtw_event_join_status_info *)evt_info;
    u8 join_status = join_info->status;
    struct rtw_event_join_fail *fail_info = &join_info->priv.fail;
    struct rtw_event_disconnect *disconn_info = &join_info->priv.disconnect;
