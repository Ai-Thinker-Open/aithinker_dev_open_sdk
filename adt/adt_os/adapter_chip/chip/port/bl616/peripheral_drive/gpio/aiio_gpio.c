...
void gpio_isr(int32_t irq, void *arg)
{
    uint8_t val1 = *(uint8_t *)arg;
    aiio_log_d("isr gpio_pin=%d\r\n", val1);
    bool intstatus = bflb_gpio_get_intstatus(io_port[val1].gpio, io_port[val1].pin);
    if (intstatus) {
        aiio_log_d("isr trigger\r\n");
        (*io_port[val1].aiio_hal_gpio_int_cb)(irq,arg);
        bflb_gpio_int_clear(io_port[val1].gpio, io_port[val1].pin);
    }
}
...
aiio_ret_t aiio_hal_gpio_deinit(uint32_t port, uint16_t pin)
{
    ...
    if (io_port[pin].irq_en)
    {
        ...
        bflb_gpio_int_mask(io_port[pin].gpio, pin, true);
    }
...
{
    (void)port;
    if (!io_port[pin].inited || io_port[pin].cfg_in_out || pin > DEF_PIN_ID_MAX)
        return AIIO_ERR_INIT_FAIL;
    if (!en)
    {
        io_port[pin].irq_en = 0;
        bflb_gpio_int_mask(io_port[pin].gpio, io_port[pin].pin, true);
        bflb_irq_disable(io_port[pin].gpio->irq_num);
    }else{
        io_port[pin].irq_en = 1;
        bflb_gpio_int_mask(io_port[pin].gpio, io_port[pin].pin, false);    
        bflb_irq_enable(io_port[pin].gpio->irq_num);
    }
    return AIIO_SUCC;
}
...
aiio_ret_t aiio_hal_gpio_int_register(uint32_t port, uint16_t pin, aiio_hal_gpio_int_cb_t cb)
{
    ...
    if (io_port[pin].cfg_pudn == GPIO_PULLDOWN)
    {
        bflb_gpio_int_init(io_port[pin].gpio, io_port[pin].pin, GPIO_INT_TRIG_MODE_ASYNC_RISING_EDGE);
    }
    else if(io_port[pin].cfg_pudn == GPIO_PULLUP)
    {
        bflb_gpio_int_init(io_port[pin].gpio, io_port[pin].pin, GPIO_INT_TRIG_MODE_SYNC_FALLING_EDGE);
    }

    bflb_gpio_int_mask(io_port[pin].gpio, io_port[pin].pin, false);

    bflb_irq_attach(io_port[pin].gpio->irq_num, gpio_isr, &io_port[pin].pin);
    bflb_irq_enable(io_port[pin].gpio->irq_num);
