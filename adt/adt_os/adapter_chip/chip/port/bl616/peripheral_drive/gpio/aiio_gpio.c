...
void gpio_isr(uint8_t pin)
{
    ...
    if (pin == io_port[pin].pin) {
        (*io_port[pin].aiio_hal_gpio_int_cb)(NULL,NULL);
    }
}
...
{
    (void)port;
    if (!io_port[pin].inited || (io_port[pin].cfg_in_out != GPIO_INPUT) || pin > DEF_PIN_ID_MAX)
        return AIIO_ERR_INIT_FAIL;
    if (!en)
    {
...
aiio_ret_t aiio_hal_gpio_int_register(uint32_t port, uint16_t pin, aiio_hal_gpio_int_cb_t cb)
{
    ...
    {
        bflb_gpio_int_init(io_port[pin].gpio, io_port[pin].pin, GPIO_INT_TRIG_MODE_SYNC_RISING_EDGE);
    }
    ...
    bflb_gpio_irq_attach(io_port[pin].pin, gpio_isr);
    bflb_irq_enable(io_port[pin].gpio->irq_num);
