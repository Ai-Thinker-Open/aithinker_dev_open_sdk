...
static rtk_bt_evt_cb_ret_t ble_gap_app_callback(uint8_t evt_code, void *param, uint32_t len)
{
    ...
    case RTK_BT_LE_GAP_EVT_SCAN_RES_IND:
    {
        ...
        uint8_t *addr_ptr = scan_res_ind->adv_report.addr.addr_val;
        uint8_t evt_type = scan_res_ind->adv_report.evt_type;

        /* Cache scannable advertising data, wait for scan response */
        if (evt_type == RTK_BT_LE_EVT_CONN_ADV ||
            evt_type == RTK_BT_LE_EVT_SCAN_UNDIR_ADV) {
            memcpy(adv_rec.addr, addr_ptr, 6);
            memcpy(adv_rec.ad_data, scan_res_ind->adv_report.data,
                scan_res_ind->adv_report.len);
            adv_rec.ad_len = scan_res_ind->adv_report.len;
            /* No callback, waiting for scan response */
            break;
        }
        ...
        if (ble_cb) {
            ...
            memcpy(evt.scan.mac, addr_ptr, 6);
            ble_reverse_byte(evt.scan.mac, 6);

            /* Handle scan response with merged advertising data */
            if (evt_type == RTK_BT_LE_EVT_SCAN_RSP) {
                evt.scan.sd_data = scan_res_ind->adv_report.data;
                evt.scan.sd_len = scan_res_ind->adv_report.len;

                /* Merge cached advertising data if address matches */
                if (memcmp(adv_rec.addr, addr_ptr, 6) == 0) {
                    evt.scan.ad_data = adv_rec.ad_data;
                    evt.scan.ad_len = adv_rec.ad_len;
                }
            } else {
                /* Direct advertising data (non-scannable) */
