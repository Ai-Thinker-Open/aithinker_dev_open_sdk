...
uint8_t aiio_set_low_power_mode(aiio_wakeup_cfg_t *wakeup_cfg)
{
	...
	switch(wakeup_cfg->wakeUpSourceMode){
		case AIIO_LOW_POWER_WAKE_UP_TIMER:	//使用定时器唤醒
	        ...
	        hal_hbn_enter((uint32_t)sleep_time);	//使用定时器作为唤醒源进入低功耗
			return 0;
		case AIIO_LOW_POWER_WAKE_UP_GPIO:	//使用GPIO唤醒
			if(6 != wakeup_cfg->wakeUpPin ){
				return 4;	//BL602仅支持IO7唤醒
			}
			...
			//printf("[%s()-%d]weakup_pin=0x%02x sleep_time=%d\r\n",__func__,__LINE__,weakup_pin,sleep_time);
			vTaskDelay(100);
