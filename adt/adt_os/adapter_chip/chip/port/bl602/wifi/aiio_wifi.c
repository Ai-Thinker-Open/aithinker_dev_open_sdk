...
static struct wifi_ap_sta_info local_wifi_ap_sta_info[CONFIG_MAX_STA_CONN_LIMIT]={0};
...
static void event_cb_wifi_event(input_event_t *event, void *private_data)
{
    ...
    switch (event->code) {
        ...
        {
            aiio_log_d("[APP] [EVT] GOT IPv6 %lld\r\n", aos_now_ms());
            aiio_log_d("[SYS] Memory left is %d Bytes\r\n", xPortGetFreeHeapSize());
            if(wifi_cb)
            {
                aiio_event.code=AIIO_WIFI_EVENT_STA_GOT_IPV6;
                wifi_cb(&aiio_event, private_data);
            }
        }
        break;
        ...
        case CODE_WIFI_ON_AP_STA_GOT_IP:
        {
            ...
            aiio_log_d("[APP] [EVT] [AP] [GET_IP] %lld, sta idx is %s", aos_now_ms(), ip4addr_ntoa(&node->ipaddr));
        }
        break;
        default:
        {
...
aiio_err_t aiio_wifi_init(void)
{
    ...
    memset(&local_wifi_ap_sta_info, 0, sizeof(local_wifi_ap_sta_info));
...
aiio_err_t aiio_wifi_set_config(aiio_wifi_port_t port,aiio_wifi_config_t *conf)
{
    ...
    if(port==AIIO_WIFI_IF_STA){  //sta config
        ...
        if(conf->sta.ssid){
            ...
            wifi_config.sta.ssid[SSID_LEN-1] = '\0';
        }
...
aiio_err_t aiio_wifi_ap_sta_list_get(uint8_t *sta_num,aiio_wifi_sta_basic_info_t *sta_info_p)
{
    ...
    for(i = 0;i < sta_cnt;i++){
        ...
        printf(" i:%d  counter:%d   sta_idx=%d, sta_mac=%02x:%02x:%02x:%02x:%02x:%02x \r\n",i,counter,
            sta_info.sta_idx, sta_info.sta_mac[0], sta_info.sta_mac[1], sta_info.sta_mac[2], sta_info.sta_mac[3], sta_info.sta_mac[4], sta_info.sta_mac[5]);
        ...
        (sta_info_p+counter)->ipv4=local_wifi_ap_sta_info[wifi_ap_get_sta_info_index((sta_info_p+counter)->sta_mac)].ip;
        (sta_info_p+counter)->rssi=sta_info.rssi;
        (sta_info_p+counter)->tsfhi=sta_info.tsfhi;
        (sta_info_p+counter)->tsflo=sta_info.tsflo;
        (sta_info_p+counter)->data_rate=sta_info.data_rate;
        (sta_info_p+counter)->is_used=sta_info.is_used;
        counter++;
    }
    *sta_num=counter;
    return AIIO_OK;
...
aiio_err_t aiio_wifi_ap_ip_set(char* ap_ip)
{
    ...
    uint8_t ip[4] = { 0 }, digit= 0, index = 0;
    char* temp_arg;
    char *at_ip_bak_p;
    char at_ip_bak[16] = {0};
    ...
    strncpy(at_ip_bak, ap_ip, sizeof(at_ip_bak));
    at_ip_bak_p = at_ip_bak;

    for (int32_t i = 0; i < 4; i++)
    {
        temp_arg = at_ip_bak_p;
        while (*(at_ip_bak_p++) != '.')
        {
            digit++;
        }
    ...
    if (!ret)
    {
        ...
        ap_ip_params.gateway = IP_SET_ADDR(ip[0], ip[1], ip[2], ip[3]);
...
aiio_err_t aiio_wifi_sta_connect_ind_stat_get(aiio_wifi_sta_connect_ind_stat_info_t *wifi_ind_stat)
{
    ...
    int ssid_len = strlen(info.ssid);
    if (ssid_len > 0) {
        memcpy(wifi_ind_stat->ssid, info.ssid, ssid_len);
        wifi_ind_stat->ssid[ssid_len] = '\0';
    }

    int passphr_len = strlen(info.passphr);
    if (passphr_len > 0) {
        memcpy(wifi_ind_stat->passphr, info.passphr, passphr_len);
        wifi_ind_stat->passphr[passphr_len] = '\0';
    }

    memcpy(wifi_ind_stat->bssid, info.bssid, 6);

    wifi_ind_stat->status_code = info.status_code;
    wifi_ind_stat->chan_band = info.chan_band;
    wifi_ind_stat->chan_id = info.chan_id;
    wifi_ind_stat->type_ind = info.type_ind;
...
#if (CFG_IPV6==1 && AIIO_CONFIG_IPV6==1) 
extern int wifi_mgmr_sta_ipv6_get(uint8_t index, ip6_addr_t *ip6addr, uint8_t *state);
int aiio_wifi_sta_ipv6_get(uint8_t index, ip_addr_t *ip6addr, uint8_t *state)
{
    return wifi_mgmr_sta_ipv6_get(index,ip6addr,state);
}
#endif
aiio_err_t aiio_wifi_set_hostname(char *hostname)
{
    return wifi_mgmr_set_hostname(hostname);
}
...
static int wifi_ap_get_sta_info_index(uint8_t mac[6])
{
    ...
    /* find exist record */
    for (i = 0; i < CONFIG_MAX_STA_CONN_LIMIT; i++) {
        if (local_wifi_ap_sta_info[i].valid_time != 0 && memcmp(local_wifi_ap_sta_info[i].mac, mac, 6) == 0) {
            return i;
        }   
    }
    ...
    /* find empty record */
    for (i = 0; i < CONFIG_MAX_STA_CONN_LIMIT; i++) {
        if (local_wifi_ap_sta_info[i].valid_time == 0) {
            return i;
        }
    }
    ...
    for (i = 0; i < CONFIG_MAX_STA_CONN_LIMIT; i++) {
        if (local_wifi_ap_sta_info[i].valid_time < oldest_time) {
            oldest_time = local_wifi_ap_sta_info[i].valid_time;
            index = i;
        }
    }
    return index;
...
static void wifi_ap_update_sta_ip(uint8_t mac[6], uint32_t ip)
{
    ...
    local_wifi_ap_sta_info[index].valid_time = aos_now_ms();
    memcpy(local_wifi_ap_sta_info[index].mac, mac, 6);
    local_wifi_ap_sta_info[index].ip = ip;
}
