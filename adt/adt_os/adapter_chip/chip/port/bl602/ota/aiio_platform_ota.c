...
int aiio_platform_ota_install(uint8_t *data, uint32_t data_len, uint32_t data_total_len)
{
    ...
    if(bin_size == 0)
    {
        if(ota_progress >= OTA_HEADER_SIZE)
        {
            ...
            ret = bl_mtd_write(handle, flash_addr_offset, data_len, data);
            if(ret){
                aiio_log_e("bl_mtd_write fail\r\n");
                aiio_platform_ota_stop();
                return AIIO_ERROR;
            }
            temp_buffer = (uint8_t *)malloc(data_len); 
            if (temp_buffer == NULL)
            {
                aiio_log_e("Failed to allocate memory for Flash read\r\n");
                aiio_platform_ota_stop();
                return AIIO_ERROR;
            }
            ret = bl_mtd_read(handle, flash_addr_offset, data_len, temp_buffer);
            if(ret){
                aiio_log_e("bl_mtd_read fail\r\n");
                free(temp_buffer);
                aiio_platform_ota_stop();
                return AIIO_ERROR;
            }
            utils_sha256_update(&ctx, temp_buffer, data_len);
            free(temp_buffer);
        ...
        else
        {
            ...
            if(ota_progress + data_len > OTA_HEADER_SIZE)
            {
                ...
                ret = bl_mtd_write(handle, flash_addr_offset, (data_len - (OTA_HEADER_SIZE - ota_progress)), &data[OTA_HEADER_SIZE - ota_progress]);
                if(ret){
                    aiio_log_e("bl_mtd_write fail\r\n");
                    aiio_platform_ota_stop();
                    return AIIO_ERROR;
                }
                temp_buffer = (uint8_t *)malloc((data_len - (OTA_HEADER_SIZE - ota_progress))); 
                if (temp_buffer == NULL)
                {
                    aiio_log_e("Failed to allocate memory for Flash read\r\n");
                    aiio_platform_ota_stop();
                ...
                ret = bl_mtd_read(handle, flash_addr_offset, (data_len - (OTA_HEADER_SIZE - ota_progress)), temp_buffer);
                if(ret){
                    aiio_log_e("bl_mtd_read fail\r\n");
                    free(temp_buffer);
                    aiio_platform_ota_stop();
                ...
                utils_sha256_update(&ctx, temp_buffer, (data_len - (OTA_HEADER_SIZE - ota_progress)));
                free(temp_buffer);
