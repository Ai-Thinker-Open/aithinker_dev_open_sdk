...
#ifdef CONFIG_RADAR_UART_ENABLE
    #define USER_UART_ENABLE                (CONFIG_RADAR_UART_ENABLE)
#else
    #define USER_UART_ENABLE                (0)
#endif

#ifdef CONFIG_RADAR_UART
    #define UART_TRAN_NUM                   (CONFIG_RADAR_UART)
#else
    #define UART_TRAN_NUM                   (1)
#endif

#ifdef CONFIG_RADAR_UART_BAUDRATE
    #define USER_UART_BAUD                  (CONFIG_RADAR_UART_BAUDRATE)
#else
    #define USER_UART_BAUD                  (256000)
#endif

#ifdef CONFIG_RADAR_FACTORY_ENABLE
    #define RADAR_FACTORY_ENABLE            (CONFIG_RADAR_FACTORY_ENABLE)
#else
    #define RADAR_FACTORY_ENABLE            (1)
#endif

#define CMD_PROCESS_TASK_PRIORITY       (13)
#define UART_RECV_CHECK_TASK_PRIORITY   (12)
#define CMD_RECV_TIMEOUT                (5)
...
//切换AT指令模式
#define RADAR_TO_AT_MODE_CMD    (0x00B2)
...
static void create_spi_init_task(void (*spi_slave_init_type)(void));
...
static uint16_t Radar_GetOneFrameChirpNum(void);
static uint16_t Radar_GetFftPoint(void);
static int hosal_spi_dma_pp_recv(hosal_spi_dev_t *arg, uint16_t len, uint8_t *ping_buf, uint8_t *pong_buf, void (*dma_rx_cb)(void *arg, uint32_t flag));
static void spi_slave_cfg(void);
static void bsp_spi_deinit(void);
static void rd01_power_off(void);
...
static rd01_cmd_ack_t cmd_ack_cb = NULL;
...
#define DFFT_PEAK_DATA_LEN  (0x6C)

static QueueHandle_t ft_queue_handle = NULL;
static TaskHandle_t ft_queue_task_handle = NULL;
static uint8_t ft_ping_buff[40];
static uint8_t ft_pong_buff[40];
...
static RADAR_REG_T ft_InitRegList[MAX_REG_NUM] __attribute__((aligned (4))) =
{
    {0x40, 0x4207},
    {0x41, 0x0004},
    {0x01, 0x0000},
...
static void DoFactoryTestDataSend(uint8_t* frameBuf, uint16_t bufLen, uint16_t index)
{
    ...
    if (index == (RADAR_CHIRP_NUM - 1))
    {
        ...
        if (data_output_cb)
        {
            data_output_cb(frameBuf, bufLen);
        }
        memset(factory_test_chirp_power, 0, sizeof(factory_test_chirp_power));
        ...
        memset(factory_test_chirp_power, 0, sizeof(factory_test_chirp_power));
    #endif//USER_UART_ENABLE
    }
}

static uint16_t Radar_GetDfftDataNum(void)
{
    uint16_t val = 32;
    
    rd01_iic_read(I2C_ADDR_BanYan_Chip0, BANYAN_DIG_DFFT_DATA_NUM, &val);

    return (val >> BANYAN_DFFT_DATA_NUM_POS);
}

static uint16_t Radar_GetDfftChirpNum(void)
{
    uint16_t val = 32;
    
    rd01_iic_read(I2C_ADDR_BanYan_Chip0, BANYAN_DIG_DFFT_CHIRP_NUM, &val);
...
static uint32_t Config_ReadRadarParaLen(void)
{
    uint32_t dataLen = 0;
...
static void Config_WriteRadarPara2Flash(uint32_t* addr, uint16_t len)
{
...
static void Config_ReadRadarParaData(uint32_t* addr, uint16_t len)
{
...
static void ft_queue_data_process_task(void *param)
{
...
static int ft_create_queue_and_task(void)
{
    dma_irq_cnt = 0;
    ...
    {
        BaseType_t err = xTaskCreate(ft_queue_data_process_task, "ft_queue_task", 512, NULL, 14, &ft_queue_task_handle);
        if (err != pdPASS)
        {
...
static void Radar_SaveRegList(void)
{
    uint16_t loop = 0;
    ...
    while(ft_InitRegList[loop].addr) 
    {
        loop++;
    }
    loop++;
    
    Config_WriteRadarPara2Flash((uint32_t*)ft_InitRegList, loop * sizeof(RADAR_REG_T));
    radarRegNeedUpDate = 0;
}

static void Ft_Radar_Init(void)
{
    uint16_t loop = 0;
    uint32_t paraLen = 0;
    ...
    printf("paraLen: 0x%x\r\n", paraLen);
    Config_ReadRadarParaData((uint32_t*)ft_InitRegList, paraLen);
    
#if 1
    while(ft_InitRegList[loop].addr)
    {
        printf("0x%02x:0x%04x\r\n", ft_InitRegList[loop].addr, ft_InitRegList[loop].val);
        loop++;
    }
    loop = 0;
#endif
    ...
    while(ft_InitRegList[loop].addr) 
    {
        rd01_iic_write(I2C_ADDR_BanYan_Chip0, (uint8_t)(ft_InitRegList[loop].addr), ft_InitRegList[loop].val);
        loop++;
    }
...
static void Ft_DataProc_Init(void)
{
...
static void ft_bsp_spi_slave_init(void)
{
    spi_slave_cfg();
...
static void Ft_System_Reconfig(void)
{
...
int32_t aiio_rd01_cmd_ack_cb_register(rd01_cmd_ack_t cb)
{
    cmd_ack_cb = cb;
    return 0;
}
	...
	switch(cmd->cmdType)
    {
		...
		case RADAR_TO_AT_MODE_CMD:
            if (CmdModeFlag != 0)
            {
                ...
                {
                    if (cmd->cmdData[0] == 0x0001)
                    {
                        //保存flash
                        char cmd_mode = 1;
                        bsp_nv_write(RADAR_AT_MODE_KET, &cmd_mode, sizeof(cmd_mode));
                    }
                    ackLen = FillCmdAck(NULL, 0, cmd->cmdType, ACK_OK);
                }
            }
...
void aiio_radar_cmd_process(uint8_t* buf, uint16_t len)
{
    uint16_t loop = 0;
...
static void CmdProc_Recv(void)
{
    ...
    aiio_radar_cmd_process(g_cmdRecv.buf[g_cmdRecv.bufProc], g_cmdRecv.bufLen);
...
static void UART_RecvTimeout(void)
{
...
static void uart_recv_timeout_check_task(void *param)
{
    for (;;)
    {
        UART_RecvTimeout();
...
int32_t aiio_rd01_init(void)
{
    ...
    xTaskCreate(uart_recv_timeout_check_task, "uart_rec_check", 1024, NULL, UART_RECV_CHECK_TASK_PRIORITY, NULL);
#endif //USER_UART_ENABLE
