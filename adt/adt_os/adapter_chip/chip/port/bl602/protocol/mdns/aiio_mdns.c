...
#if AIIO_CONFIG_IPV6
static char* cur_ipv6=NULL;
static char* pre_ipv6=NULL;
#endif
...
#if AIIO_CONFIG_IPV6
    cur_ipv6 = inet6_ntoa(netif->ip_addr.u_addr.ip6);
    memcpy(pre_ipv6,cur_ipv6,strlen(cur_ipv6));
    aiio_log_d("pre_ipv6 = %s ", pre_ipv6);
    aiio_log_d("cur_ipv6 = %s ", cur_ipv6);
#else
...
#if AIIO_CONFIG_IPV6
    cur_ipv6 = inet6_ntoa(netif->ip_addr.u_addr.ip6);
    if (strcmp(cur_ipv6,pre_ipv6) != 0)
    {
        aiio_log_i("pre_ip = %s ", pre_ipv6);
        aiio_log_i("cur_ip = %s ", cur_ipv6);
