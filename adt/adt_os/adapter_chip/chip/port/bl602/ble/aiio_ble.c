...
#define CREATE_FLAG(flag) static int flag = 0
...
CREATE_FLAG(flag_indicate_complete);
...
static struct bt_uuid_128 srv_uuid_bulfi[1];
static struct bt_uuid_128 tx_uuid_bulfi[2];
static struct bt_uuid_128 rx_uuid_bulfi[2];
...
static struct bt_gatt_attr salve_server_blufi[]= {
    /* Primary Service */
    BT_GATT_PRIMARY_SERVICE(&srv_uuid_bulfi[0]),

    /* Characteristic && Characteristic User Declaration */
    BT_GATT_CHARACTERISTIC(&rx_uuid_bulfi[0],
                   BT_GATT_CHRC_WRITE | BT_GATT_CHRC_WRITE_WITHOUT_RESP,
                   BT_GATT_PERM_WRITE, NULL, ble_srv1_write_val,
                   NULL),

    /* Characteristic && Characteristic User Declaration */
    BT_GATT_CHARACTERISTIC(&tx_uuid_bulfi[0],
                   BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                   BT_GATT_PERM_READ, NULL, NULL,
                   NULL),
    BT_GATT_CCC(ble_ccc_cfg_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
...
static struct bt_gatt_attr salve_server_blufi_2[]= {
    /* Primary Service */
    BT_GATT_PRIMARY_SERVICE(&srv_uuid[1]),

    /* Characteristic && Characteristic User Declaration */
    BT_GATT_CHARACTERISTIC(&tx_uuid_bulfi[1],
                   BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY | BT_GATT_CHRC_INDICATE,
                   BT_GATT_PERM_READ, NULL, NULL,
                   NULL),
    BT_GATT_CCC(ble_ccc_cfg_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
    ...
    /* Characteristic && Characteristic User Declaration */
    BT_GATT_CHARACTERISTIC(&rx_uuid_bulfi[1],
                   BT_GATT_CHRC_WRITE | BT_GATT_CHRC_WRITE_WITHOUT_RESP,
                   BT_GATT_PERM_WRITE, NULL, ble_srv2_write_val,
                   NULL),
};
...
static struct bt_gatt_service ble_server_blufi = BT_GATT_SERVICE(salve_server_blufi);
static struct bt_gatt_service ble_server_blufi_2 = BT_GATT_SERVICE(salve_server_blufi_2);
...
static void _connected(struct bt_conn *conn, u8_t err)
{
    ...
    if (ble_config_s.smp_cfg.pairmode == AIIO_BLE_PAIRING_MODE_PAIRABLE) {
        if (conn->role == BT_CONN_ROLE_SLAVE) {
            bt_passkey_set(ble_config_s.smp_cfg.passkey);
            bt_conn_set_security(conn, BT_SECURITY_L2);
        }
    }
#endif
    #if CONFIG_AIIO_SLAVE_BLE_MTU_EN
                        ...
                        u8_t err)
{
    ...
    UNSET_FLAG(flag_indicate_complete);
}
static int ble_indicate_data(void *conn, struct bt_gatt_attr *char_val, uint8_t *data, uint16_t length)
{
    int ret;
    uint16_t mtu;
    ...
    WAIT_FOR_FLAG(flag_indicate_complete);
    ...
    /* set indicate state */
    SET_FLAG(flag_indicate_complete);
    /* send data */
...
static int32_t ble_init_flag;
static volatile int32_t ble_deinit_in_progress;
...
{
    if (!ble_init_flag || ble_deinit_in_progress) {
        return AIIO_ERROR;
    }
...
aiio_err_t aiio_ble_send(const aiio_ble_send_t *data)
{
    ...
    switch (data->type) {
        ...
        case AIIO_BLE_INDICATE:
            ble_indicate_data(data->conn, (struct bt_gatt_attr *)data->handle, data->data, data->length);
            break;
...
aiio_err_t ble_gatts_add_blufi_svcs(aiio_ble_default_server_t *cfg, aiio_ble_default_handle_t *handle)
{
    ...
    ble_uuid_convert(&srv_uuid_bulfi[0], cfg->server_uuid[0]);
    ble_uuid_convert(&tx_uuid_bulfi[0], cfg->tx_char_uuid[0]);
    ble_uuid_convert(&rx_uuid_bulfi[0], cfg->rx_char_uuid[0]);

    ble_uuid_convert(&srv_uuid[1], cfg->server_uuid[1]);
    ble_uuid_convert(&tx_uuid_bulfi[1], cfg->tx_char_uuid[1]);
    ble_uuid_convert(&rx_uuid_bulfi[1], cfg->rx_char_uuid[1]);

    ret = bt_gatt_service_register(&ble_server_blufi);
    ret |= bt_gatt_service_register(&ble_server_blufi_2);

    if (ret != 0) {
        // return AIIO_ERROR;
    }

    handle->tx_char_handle[0] = (aiio_ble_att_handle_t)&salve_server_blufi[4];
    handle->rx_char_handle[0] = (aiio_ble_att_handle_t)&salve_server_blufi[2];

    handle->tx_char_handle[1] = (aiio_ble_att_handle_t)&salve_server_blufi_2[SRV_TX_INDEX];
    handle->rx_char_handle[1] = (aiio_ble_att_handle_t)&salve_server_blufi_2[SRV_RX_INDEX];

    return AIIO_OK;
}

static void ccc_write_cb(struct bt_conn *conn, u8_t err,
		       struct bt_gatt_write_params *params)
{
	aiio_log_i("[CCC] Write err %u \r\n", err);
...
aiio_err_t ble_gattc_ccc_cfg(const aiio_ble_ccc_t *cfg)
{
    ...
    write_params.handle = cfg->handle;
    write_params.offset = 0;
    write_params.data = (void *)&cfg->cfg;
    write_params.length = 2;

    UNSET_FLAG(flag_write_complete);
    ret = bt_gatt_write(cfg->conn, &write_params);
    if (ret != 0) {
        return AIIO_ERROR;
    }
