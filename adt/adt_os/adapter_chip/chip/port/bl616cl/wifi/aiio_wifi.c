...
static wifi_mgmr_sniffer_item_t sniffer_item;
...
static long long aos_now_ms(void)
{
    long long ms;
...
void wifi_event_handler(async_input_event_t ev, void *priv)
{
    ...
    wifi_config_queue_data.value = priv;
    switch (code) 
    {
    ...
    while (1)
    {
        ...
        switch (wifi_config_queue_data.evt) 
        {
            ...
            case CODE_WIFI_ON_AP_STA_ADD: {
                LOG_I("[APP] [EVT] [AP] [ADD] code2:%u\r\n", wifi_config_queue_data.value);
                if(wifi_cb)
                {
                    aiio_event.code=AIIO_WIFI_EVENT_AP_STACONNECTED;
                    aiio_event.value=wifi_config_queue_data.value;
                    wifi_cb(&aiio_event, 0);
                }
            } break;
            case CODE_WIFI_ON_AP_STA_DEL: {
                LOG_I("[APP] [EVT] [AP] [DEL] code2:%u\r\n", wifi_config_queue_data.value);
                if(wifi_cb)
                {
            ...
            case CODE_WIFI_ON_AP_STA_GOT_IP: {
                LOG_I("[APP] [EVT] [AP] [GET_IP]\r\n");
                extern struct dhcp_client_node* g_aiio_node;
                struct dhcp_client_node *node = g_aiio_node;
                wifi_ap_update_sta_ip((uint8_t *)node->chaddr, (uint32_t)node->ipaddr.addr);
                // LOG_I("[APP] [EVT] [AP] [GET_IP] %lld\r\n,sta idx is %s", xTaskGetTickCount(),ip4addr_ntoa(&node->ipaddr));
            } break;
            default: {
...
aiio_err_t aiio_wifi_connect(void)
{
    uint16_t freq = 0;
    char bssid[18] = {0};
    sprintf(&bssid,"%02x:%02x:%02x:%02x:%02x:%02x",wifi_config.sta.bssid[0],wifi_config.sta.bssid[1],wifi_config.sta.bssid[2],
    ...
    if(g_wifi_sta_interface){
        ...
        return wifi_sta_connect((char*)wifi_config.sta.ssid, (char*)wifi_config.sta.password,
                                bssid,NULL,1,freq,freq,wifi_config.sta.use_dhcp);
    }
    return AIIO_ERROR;
}
...
aiio_err_t aiio_wifi_get_wifi_channel_conf(aiio_wifi_conf_t *wifi_chan_conf)
{
    ...
    wifi_chan_conf->channel_nums = wifiMgmr.channel24G_num;
    return AIIO_OK;
}
...
aiio_err_t aiio_wifi_ap_ip_set(char* ap_ip)
{
    ...
    ip_cfg.ipv4.dns = ap_ip_params.gateway;
...
aiio_err_t aiio_wifi_sta_connect_ind_stat_get(aiio_wifi_sta_connect_ind_stat_info_t *wifi_ind_stat)
{
    ...
    if(wifi_mgmr_sta_connect_ind_stat_get(&cur_wifi_ind_stat)==AIIO_OK){
...
{
    return net_set_hostname(hostname);
}
