...
int aiio_create_mbedtls_handle(const char *hostname, size_t hostlen, const void *cfg, aiio_tls_t *tls)
{
    ...
    #else
    mbedtls_ssl_conf_rng(&tls->conf, ai_random, NULL);
    #endif
...
extern int aiio_hal_random(void *p_rng,uint8_t *output,uint32_t output_len);
...
#else
            ret = mbedtls_pk_parse_key(pki->pk_key, pki->privkey_pem_buf, pki->privkey_pem_bytes,
                                       pki->privkey_password, pki->privkey_password_len, aiio_hal_random, NULL);
#endif
        } else {
