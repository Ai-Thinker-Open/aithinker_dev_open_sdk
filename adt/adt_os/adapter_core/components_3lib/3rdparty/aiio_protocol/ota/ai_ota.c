...
void ai_https_update_ota(void *param)
{
    ...
    aiio_log_i("mbedtls_entropy_init\r\n");
    if((ret = mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy,
                                    NULL, 0)) != 0)
    {
        aiio_log_i("mbedtls_ctr_drbg_seed returned %d\r\n", ret);
        goto exit;
    }
    aiio_log_i("mbedtls_ctr_drbg_seed success\r\n");
#endif
    ...
    {
        aiio_log_i("mbedtls_ssl_config_defaults returned %d\r\n", ret);
        goto exit;
    }
    ...
    {
        aiio_log_i("mbedtls_ssl_setup returned -0x%x\n\n", -ret);
        goto exit;
    }
    aiio_log_i("mbedtls_ssl_setup sucess\r\n");

    if((ret = mbedtls_net_connect(&server_fd, ota_parame_t->host, port_str,
            MBEDTLS_NET_PROTO_TCP)) != 0)
    {
        aiio_log_i("mbedtls_net_connect returned -%x\r\n", -ret);
        goto exit;
    }
    aiio_log_i("mbedtls_net_connect sucess\r\n");
    ...
    while((ret = mbedtls_ssl_handshake(&ssl)) != 0)
    {
        ...
        {
            aiio_log_i("mbedtls_ssl_handshake returned -0x%x\r\n", -ret);
            goto exit;
        }
    }
    aiio_log_i("mbedtls_ssl_handshake sucess\r\n");
    
    aiio_log_i("SSL ciphersuite %s\n", mbedtls_ssl_get_ciphersuite(&ssl));
    ...
    // aiio_log_i("request = %s\r\n", request);

    ret = mbedtls_ssl_write(&ssl, request, strlen((char *)request));
    if(ret < 0){
        aiio_log_i("mbedtls_ssl_write returned -0x%x\r\n", -ret);
        goto exit;
    }
    aiio_log_i("mbedtls_ssl_write success\r\n");
    ...
    while(3 >= rsp_result.parse_status)
    {
        if(0 == rsp_result.parse_status){//didn't get the http response
			...
			if(read_bytes <= 0){
				aiio_log_i("mbedtls_ssl_read returned -0x%x\r\n", -read_bytes);
				goto exit;
			}
            idx = read_bytes;
    ...
    do
    {
        ...
        if(ret < 0)
        {
            aiio_log_i("mbedtls_ssl_read returned -0x%x", -ret);
            break;
        }

        if(ret == 0)
        {
            aiio_log_i("connection closed");
            break;
        }
    ...
    if(0 == memcmp(md5, pack_head_t.md5, sizeof(&pack_head_t.md5))){
        ...
        for(int i=0; i<16; i++)
            aiio_log_i("%02X", md5[i]);
        aiio_log_i("\r\n");
        ota_parame_t->set_boot_partition_cb();
    }else{
        aiio_log_i("MD5 verify failed\r\n");
        goto exit;
    }
...
static int connect_server(int server_socket, char *host, int port)
{
	...
	// if(server == NULL){ 
	// 	aiio_log_i("[ERROR] Get host ip failed\n");
	// 	return -1;
	// }
...
void ai_http_update_ota(void *param)
{
    ...
    if(ret < 0){
        aiio_log_i("send http requst failed\r\n");
        goto exit;
    }
    ...
    while(3 >= rsp_result.parse_status)
    {
        if(0 == rsp_result.parse_status){//didn't get the http response
			...
			if(read_bytes <= 0){
				aiio_log_i("read socket failed\r\n");
				goto exit;
			}
            // for(int i=0; i<read_bytes; i++){
            //     aiio_log_i("%02X ", recv_buf[i]);
            // }
            // aiio_log_i("\r\n");
            aiio_log_i("len = %d\r\n", read_bytes);
            aiio_log_i("http head = %s\r\n", recv_buf);
            idx = read_bytes;
			memset(&rsp_result, 0, sizeof(rsp_result));
			if(ai_parse_http_response(recv_buf, read_bytes, &rsp_result) == -1){
				goto exit;
    ...
    if(0 == rsp_result.body_len)
    {
        ...
        aiio_log_i("Download new firmware begin, total size : %d\n", rsp_result.body_len);
    }
    ...
    if(writelen != 0){
        ...
        if(read_bytes <= 0){
            aiio_log_i("recv ota data failed\r\n");
            goto exit;
        }
        len = read_bytes;
    }
