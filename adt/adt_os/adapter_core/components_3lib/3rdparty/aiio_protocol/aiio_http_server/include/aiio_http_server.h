...
#ifndef _AIIO_HTTP_SERVER_H_
#define _AIIO_HTTP_SERVER_H_
...
/*
note: aiio_https_server.h includes a customized copy of this
...
#define AIIO_ERR_HTTPD_BASE (0xb000)                            /*!< Starting number of HTTPD error codes */
#define AIIO_ERR_HTTPD_HANDLERS_FULL (AIIO_ERR_HTTPD_BASE + 1)  /*!< All slots for registering URI handlers have been consumed */
#define AIIO_ERR_HTTPD_HANDLER_EXISTS (AIIO_ERR_HTTPD_BASE + 2) /*!< URI handler with same method and target URI already registered */
#define AIIO_ERR_HTTPD_INVALID_REQ (AIIO_ERR_HTTPD_BASE + 3)    /*!< Invalid request pointer */
#define AIIO_ERR_HTTPD_RESULT_TRUNC (AIIO_ERR_HTTPD_BASE + 4)   /*!< Result string truncated */
#define AIIO_ERR_HTTPD_RESP_HDR (AIIO_ERR_HTTPD_BASE + 5)       /*!< Response header field larger than supported */
#define AIIO_ERR_HTTPD_RESP_SEND (AIIO_ERR_HTTPD_BASE + 6)      /*!< Error occured while sending response packet */
#define AIIO_ERR_HTTPD_ALLOC_MEM (AIIO_ERR_HTTPD_BASE + 7)      /*!< Failed to dynamically allocate memory for resource */
#define AIIO_ERR_HTTPD_TASK (AIIO_ERR_HTTPD_BASE + 8)           /*!< Failed to launch server task/thread */

/* Symbol to be used as length parameter in aiio_httpd_resp_send APIs
...
#define HTTPD_RESP_USE_STRLEN -1
     ...
     */
    typedef void *aiio_httpd_handle_t;

    /**
     * @brief   HTTP Method Type wrapper over "enum aiio_http_method"
     *          available in "http_parser" library
     */
    typedef enum http_method aiio_httpd_method_t;

    /**
     * @brief  Prototype for freeing context data (if any)
     * @param[in] ctx   object to free
     */
    typedef void (*aiio_httpd_free_ctx_fn_t)(void *ctx);
     ...
     * @return
     *  - AIIO_OK   : On success
     *  - Any value other than AIIO_OK will signal the server to close the socket immediately
     */
    typedef aiio_err_t (*aiio_httpd_open_func_t)(aiio_httpd_handle_t hd, int sockfd);
     ...
     * @param[in] sockfd   session socket file descriptor
     */
    typedef void (*aiio_httpd_close_func_t)(aiio_httpd_handle_t hd, int sockfd);
     ...
     */
    typedef bool (*aiio_httpd_uri_match_func_t)(const char *reference_uri,
     ...
     */
    typedef struct aiio_httpd_config
    {
         ...
         * The value can be retrieved using the server handle, available e.g. in the aiio_httpd_req_t struct.
         *
         ...
         */
        aiio_httpd_free_ctx_fn_t global_user_ctx_free_fn;
         ...
         */
        aiio_httpd_free_ctx_fn_t global_transport_ctx_free_fn;
         ...
         * aiio_httpd_sess_set_transport_ctx() and retrieve it later with aiio_httpd_sess_get_transport_ctx()
         *
         * Returning a value other than AIIO_OK will immediately close the new socket.
         */
        aiio_httpd_open_func_t open_fn;
         ...
         */
        aiio_httpd_close_func_t close_fn;
         ...
         *        a new URI handler using `aiio_httpd_register_uri_handler()`
         *
         * Available options are:
         *     1) NULL : Internally do basic matching using `strncmp()`
         *     2) `aiio_httpd_uri_match_wildcard()` : URI wildcard matcher
         *
         * Users can implement their own matching functions (See description
         * of the `aiio_httpd_uri_match_func_t` function prototype)
         */
        aiio_httpd_uri_match_func_t uri_match_fn;
    } aiio_httpd_config_t;
     ...
     * aiio_httpd_handle_t start_webserver(void)
     * {
     ...
     *          aiio_httpd_register_uri_handler(server, &uri_get);
     *          aiio_httpd_register_uri_handler(server, &uri_post);
     *      }
     ...
     * @return
     *  - AIIO_OK    : Instance created successfully
     *  - AIIO_ERR_INVALID_ARG      : Null argument(s)
     *  - AIIO_ERR_HTTPD_ALLOC_MEM  : Failed to allocate memory for instance
     *  - AIIO_ERR_HTTPD_TASK       : Failed to launch server task
     */
     ...
     * void stop_webserver(aiio_httpd_handle_t server)
     * {
     *      // Ensure handle is non NULL
     *      if (server != NULL) {
     *          // Stop the httpd server
     *          aiio_httpd_stop(server);
     *      }
     * }
     *
     * @endcode
     ...
     * @return
     *  - AIIO_OK : Server stopped successfully
     *  - AIIO_ERR_INVALID_ARG : Handle argument is Null
     */
...
#define CONFIG_HTTPD_MAX_REQ_HDR_LEN 1024 // Max HTTP Request Header Length

#define CONFIG_HTTPD_MAX_URI_LEN 1024 // Max HTTP URI Length

#define CONFIG_HTTPD_ERR_RESP_NO_DELAY 1 // Use TCP_NODELAY socket option when sending HTTP error responses"

#define CONFIG_HTTPD_PURGE_BUF_LEN 32 // Length of temporary buffer for purging data
...
#define HTTPD_MAX_URI_LEN CONFIG_HTTPD_MAX_URI_LEN
     ...
     */
    typedef struct aiio_httpd_req
    {
        aiio_httpd_handle_t handle;                 /*!< Handle to server instance */
         ...
         */
        aiio_httpd_free_ctx_fn_t free_ctx;
     ...
     */
    typedef struct aiio_httpd_uri
    {
        const char *uri;       /*!< The URI to handle */
        aiio_httpd_method_t method; /*!< Method supported by the URI */

        /**
         * Handler to call for supported request method. This must
         * return AIIO_OK, or else the underlying socket will be closed.
         */
        aiio_err_t (*handler)(aiio_httpd_req_t *r);
     ...
     * @code{c}
     *
     * aiio_err_t my_uri_handler(aiio_httpd_req_t* req)
     * {
     ...
     *
     * // Register handler
     * if (aiio_httpd_register_uri_handler(server_handle, &my_uri) != AIIO_OK) {
     ...
     *
     * @return
     *  - AIIO_OK : On successfully registering the handler
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     *  - AIIO_ERR_HTTPD_HANDLERS_FULL  : If no slots left for new handler
     *  - AIIO_ERR_HTTPD_HANDLER_EXISTS : If handler with same URI and
     ...
     *
     * @return
     *  - AIIO_OK : On successfully deregistering the handler
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     *  - AIIO_ERR_NOT_FOUND   : Handler with specified URI and method not found
     */
     ...
     *
     * @return
     *  - AIIO_OK : On successfully deregistering all such handlers
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     *  - AIIO_ERR_NOT_FOUND   : No handler registered with specified uri string
     */
     ...
     *    error response codes, therefore, aiio_httpd_resp_send_err() must be
     *    invoked inside this function if user wishes to generate HTTP
     *    error responses.
     *  - When invoked, the validity of `uri`, `method`, `content_len`
     *    and `user_ctx` fields of the aiio_httpd_req_t parameter is not
     *    guaranteed as the HTTP request may be partially received/parsed.
     *  - The function must return AIIO_OK if underlying socket needs to
     ...
     */
    typedef aiio_err_t (*aiio_httpd_err_handler_func_t)(aiio_httpd_req_t *req,
     ...
     * given by `aiio_httpd_err_code_t`. See prototype `aiio_httpd_err_handler_func_t`
     * above for details.
     *
     ...
     *
     * @return
     *  - AIIO_OK : handler registered successfully
     *  - AIIO_ERR_INVALID_ARG : invalid error code or server handle
     */
    aiio_err_t aiio_httpd_register_err_handler(aiio_httpd_handle_t handle,
                                         aiio_httpd_err_code_t error,
                                         aiio_httpd_err_handler_func_t handler_fn);
...
#define HTTPD_SOCK_ERR_TIMEOUT -3
     ...
     *         return value of aiio_httpd_send() function
     *
     * @param[in] hd        server instance
     * @param[in] sockfd    session socket file descriptor
     ...
     */
    typedef int (*aiio_httpd_send_func_t)(aiio_httpd_handle_t hd, int sockfd, const char *buf, size_t buf_len, int flags);
     ...
     *         depending upon the set value of errno, and return specific
     *         HTTPD_SOCK_ERR_ codes, which will eventually be conveyed as
     *         return value of aiio_httpd_req_recv() function
     *
     * @param[in] hd        server instance
     * @param[in] sockfd    session socket file descriptor
     * @param[in] buf       buffer with bytes to send
     ...
     */
    typedef int (*aiio_httpd_recv_func_t)(aiio_httpd_handle_t hd, int sockfd, char *buf, size_t buf_len, int flags);
     ...
     */
    typedef int (*aiio_httpd_pending_func_t)(aiio_httpd_handle_t hd, int sockfd);
     ...
     * a URI handler where aiio_httpd_req_t* request pointer is valid.
     * @{
     */
     ...
     *          - a URI handler where sockfd is obtained using aiio_httpd_req_to_sockfd()
     *
     ...
     *
     * @return
     *  - AIIO_OK : On successfully registering override
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     */
     ...
     *      aiio_httpd_sess_get_ctx(),
     *      aiio_httpd_sess_trigger_close(),
     *      aiio_httpd_sess_update_lru_counter().
     *
     ...
     * provided buffer. Use content_len provided in aiio_httpd_req_t structure
     ...
     *  - This API is supposed to be called only from the context of
     *    a URI handler where aiio_httpd_req_t* request pointer is valid.
     *  - Once aiio_httpd_resp_send() API is called all request headers
     ...
     *  - Use aiio_httpd_req_get_hdr_value_len() to know the right buffer length
     *
     * @param[in]  r        The request being responded to
     ...
     *
     * @return
     *  - AIIO_OK : Field found in the request header and value string copied
     *  - AIIO_ERR_NOT_FOUND          : Key not found
     *  - AIIO_ERR_INVALID_ARG        : Null arguments
     *  - AIIO_ERR_HTTPD_INVALID_REQ  : Invalid HTTP request pointer
     *  - AIIO_ERR_HTTPD_RESULT_TRUNC : Value string truncated
     */
     ...
     *    aiio_httpd_req_get_hdr_value_str() to know the 'Content-Type' (eg. Content-Type:
     ...
     *  - If output size is greater than input, then the value is truncated,
     *    accompanied by truncation error as return value
     *  - Prior to calling this function, one can use aiio_httpd_req_get_url_query_len()
     ...
     *
     * @return
     *  - AIIO_OK : Query is found in the request URL and copied to buffer
     *  - AIIO_ERR_NOT_FOUND          : Query not found
     *  - AIIO_ERR_INVALID_ARG        : Null arguments
     *  - AIIO_ERR_HTTPD_INVALID_REQ  : Invalid HTTP request pointer
     *  - AIIO_ERR_HTTPD_RESULT_TRUNC : Query string truncated
     */
     ...
     *
     * @return
     *  - AIIO_OK : Key is found in the URL query string and copied to buffer
     *  - AIIO_ERR_NOT_FOUND          : Key not found
     *  - AIIO_ERR_INVALID_ARG        : Null arguments
     *  - AIIO_ERR_HTTPD_RESULT_TRUNC : Value string truncated
     */
    aiio_err_t aiio_httpd_query_key_value(const char *qry, const char *key, char *val, size_t val_size);
     ...
     *                                  AIIO_OK is returned and required buffer length incase AIIO_ERR_HTTPD_RESULT_TRUNC is returned.
     *
     * @return
     *  - AIIO_OK : Key is found in the cookie string and copied to buffer
     *  - AIIO_ERR_NOT_FOUND          : Key not found
     *  - AIIO_ERR_INVALID_ARG        : Null arguments
     *  - AIIO_ERR_HTTPD_RESULT_TRUNC : Value string truncated
     *  - AIIO_ERR_NO_MEM             : Memory allocation failure
     */
     ...
     */
    bool aiio_httpd_uri_match_wildcard(const char *uri_template, const char *uri_to_match, size_t match_upto);
     ...
     * aiio_httpd_resp_send_chunk() instead.
     *
     ...
     *      aiio_httpd_resp_set_status() - for setting the HTTP status string,
     *      aiio_httpd_resp_set_type()   - for setting the Content Type,
     *      aiio_httpd_resp_set_hdr()    - for appending any additional field
     ...
     *
     * @return
     *  - AIIO_OK : On successfully sending the response packet
     *  - AIIO_ERR_INVALID_ARG : Null request pointer
     *  - AIIO_ERR_HTTPD_RESP_HDR    : Essential headers are too large for internal buffer
     *  - AIIO_ERR_HTTPD_RESP_SEND   : Error in raw send
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request
     */
     ...
     * a single buffer, please use aiio_httpd_resp_send() instead.
     *
     ...
     *
     * @return
     *  - AIIO_OK : On successfully sending the response packet chunk
     *  - AIIO_ERR_INVALID_ARG : Null request pointer
     *  - AIIO_ERR_HTTPD_RESP_HDR    : Essential headers are too large for internal buffer
     *  - AIIO_ERR_HTTPD_RESP_SEND   : Error in raw send
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request pointer
     */
     ...
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request
     */
    static inline aiio_err_t aiio_httpd_resp_sendstr(aiio_httpd_req_t *r, const char *str)
    {
        return aiio_httpd_resp_send(r, str, (str == NULL) ? 0 : HTTPD_RESP_USE_STRLEN);
    }
     ...
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request
     */
    static inline aiio_err_t aiio_httpd_resp_sendstr_chunk(aiio_httpd_req_t *r, const char *str)
    {
        return aiio_httpd_resp_send_chunk(r, str, (str == NULL) ? 0 : HTTPD_RESP_USE_STRLEN);
    }
...
#define HTTPD_TYPE_OCTET "application/octet-stream" /*!< HTTP Content type octext-stream */
     ...
     *
     * @return
     *  - AIIO_OK : On successfully appending new header
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     *  - AIIO_ERR_HTTPD_RESP_HDR    : Total additional headers exceed max allowed
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request pointer
     */
     ...
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request pointer
     */
    aiio_err_t aiio_httpd_resp_send_err(aiio_httpd_req_t *req, aiio_httpd_err_code_t error, const char *msg);
     ...
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request pointer
     */
    static inline aiio_err_t aiio_httpd_resp_send_404(aiio_httpd_req_t *r)
    {
        return aiio_httpd_resp_send_err(r, HTTPD_404_NOT_FOUND, NULL);
    }
     ...
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request pointer
     */
    static inline aiio_err_t aiio_httpd_resp_send_408(aiio_httpd_req_t *r)
    {
        return aiio_httpd_resp_send_err(r, HTTPD_408_REQ_TIMEOUT, NULL);
    }
     ...
     *  - AIIO_ERR_HTTPD_INVALID_REQ : Invalid request pointer
     */
    static inline aiio_err_t aiio_httpd_resp_send_500(aiio_httpd_req_t *r)
    {
        return aiio_httpd_resp_send_err(r, HTTPD_500_INTERNAL_SERVER_ERROR, NULL);
    }
     ...
     *          aiio_httpd_resp_send(),
     *          aiio_httpd_resp_send_chunk()
     *
     * @param[in] r         The request being responded to
     ...
     * aiio_httpd_sess_set_send_override().
     *
     * @param[in] hd        server instance
     * @param[in] sockfd    session socket file descriptor
     * @param[in] buf       buffer with bytes to send
     ...
     * aiio_httpd_sess_set_recv_override().
     *
     * @param[in] hd        server instance
     * @param[in] sockfd    session socket file descriptor
     * @param[in] buf       buffer with bytes to send
     ...
     * through the aiio_httpd_req_t structure. But, there are cases where the web
     ...
     *
     * @return
     *  - AIIO_OK : Socket found and LRU counter updated
     *  - AIIO_ERR_NOT_FOUND   : Socket not found
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     */
     ...
     *       aiio_httpd_config_t structure.
     *
     * @return
     *  - AIIO_OK              : Successfully retrieved session list
     *  - AIIO_ERR_INVALID_ARG : Wrong arguments or list is longer than provided array
     */
     ...
     *          Please refer to aiio_httpd_queue_work() for more details.
     * @param[in] arg   The arguments for this work function
     */
    typedef void (*aiio_httpd_work_fn_t)(void *arg);
     ...
     *
     * @return
     *  - AIIO_OK   : On successfully queueing the work
     *  - AIIO_FAIL : Failure in ctrl socket
     *  - AIIO_ERR_INVALID_ARG : Null arguments
     */
 ...
 */
    ...
    } aiio_httpd_ws_client_info_t;

    /**
     * @brief WebSocket frame format
     */
    typedef struct aiio_httpd_ws_frame
    {
        bool final;           /*!< Final frame:
                                   ...
                                   For frames to be transmitted, this field is only used if the `fragmented`
                                       ...
                                       (aiio_http_server doesn't automatically fragment messages) */
        ...
        aiio_httpd_ws_type_t type; /*!< WebSocket frame type */
        uint8_t *payload;     /*!< Pre-allocated data buffer */
        size_t len;           /*!< Length of the WebSocket data */
    } aiio_httpd_ws_frame_t;

    /**
     * @brief Transfer complete callback
     */
    typedef void (*aiio_transfer_complete_cb)(aiio_err_t err, int socket, void *arg);

    /**
     * @brief Receive and parse a WebSocket frame
     *
     * @note    Calling aiio_httpd_ws_recv_frame() with max_len as 0 will give actual frame size in pkt->len.
     *          The user can dynamically allocate space for pkt->payload as per this length and call aiio_httpd_ws_recv_frame() again to get the actual data.
     ...
     * @return
     *  - AIIO_OK                    : On successful
     *  - AIIO_FAIL                  : Socket errors occurs
     *  - AIIO_ERR_INVALID_STATE     : Handshake was already done beforehand
     *  - AIIO_ERR_INVALID_ARG       : Argument is invalid (null or non-WebSocket)
     */
    aiio_err_t aiio_httpd_ws_recv_frame(aiio_httpd_req_t *req, aiio_httpd_ws_frame_t *pkt, size_t max_len);
     ...
     * @return
     *  - AIIO_OK                    : On successful
     *  - AIIO_FAIL                  : When socket errors occurs
     *  - AIIO_ERR_INVALID_STATE     : Handshake was already done beforehand
     *  - AIIO_ERR_INVALID_ARG       : Argument is invalid (null or non-WebSocket)
     */
    aiio_err_t aiio_httpd_ws_send_frame(aiio_httpd_req_t *req, aiio_httpd_ws_frame_t *pkt);
     ...
     *
     * This API should rarely be called directly, with an exception of asynchronous send using aiio_httpd_queue_work.
     *
     ...
     *  - AIIO_OK                    : On successful
     *  - AIIO_FAIL                  : When socket errors occurs
     *  - AIIO_ERR_NO_MEM            : Unable to allocate memory
     */
