...
typedef struct aiio_http_client *aiio_http_client_handle_t;
typedef struct aiio_http_client_event *aiio_http_client_event_handle_t;
...
typedef enum {
    ...
    HTTP_EVENT_HEADER_SENT = HTTP_EVENT_HEADERS_SENT, /*!< This header has been kept for backward compatability
                                                           and will be deprecated in future versions aiio-idf */
...
} aiio_http_client_event_id_t;

/**
 * @brief      HTTP Client events data
 */
typedef struct aiio_http_client_event {
    aiio_http_client_event_id_t event_id;    /*!< event_id, to know the cause of the event */
    aiio_http_client_handle_t client;        /*!< aiio_http_client_handle_t context */
    void *data;                             /*!< data of the event */
    int data_len;                           /*!< data length of data */
    void *user_data;                        /*!< user_data context, from aiio_http_client_config_t user_data */
...
} aiio_http_client_transport_t;

typedef int (*http_event_handle_cb)(aiio_http_client_event_t *evt);
...
typedef struct {
    ...
    int                         port;                /*!< Port to connect, default depend on aiio_http_client_transport_t (80 or 443) */
    const char                  *username;           /*!< Using for Http authentication */
    const char                  *password;           /*!< Using for Http authentication */
    aiio_http_client_auth_type_t auth_type;           /*!< Http authentication type, see `aiio_http_client_auth_type_t` */
    ...
    aiio_http_client_method_t    method;                   /*!< HTTP Method */
    ...
    aiio_http_client_transport_t transport_type;           /*!< HTTP transport type, see `aiio_http_client_transport_t` */
    ...
    int (*crt_bundle_attach)(void *conf);      /*!< Function pointer to aiio_crt_bundle_attach. Enables the use of certification
...
#define AIIO_ERR_HTTP_BASE               (0x7000)                    /*!< Starting number of HTTP error codes */
#define AIIO_ERR_HTTP_MAX_REDIRECT       (AIIO_ERR_HTTP_BASE + 1)     /*!< The error exceeds the number of HTTP redirects */
#define AIIO_ERR_HTTP_CONNECT            (AIIO_ERR_HTTP_BASE + 2)     /*!< Error open the HTTP connection */
#define AIIO_ERR_HTTP_WRITE_DATA         (AIIO_ERR_HTTP_BASE + 3)     /*!< Error write HTTP data */
#define AIIO_ERR_HTTP_FETCH_HEADER       (AIIO_ERR_HTTP_BASE + 4)     /*!< Error read HTTP header from server */
#define AIIO_ERR_HTTP_INVALID_TRANSPORT  (AIIO_ERR_HTTP_BASE + 5)     /*!< There are no transport support for the input scheme */
#define AIIO_ERR_HTTP_CONNECTING         (AIIO_ERR_HTTP_BASE + 6)     /*!< HTTP connection hasn't been established yet */
#define AIIO_ERR_HTTP_EAGAIN             (AIIO_ERR_HTTP_BASE + 7)     /*!< Mapping of errno EAGAIN to int */
#define AIIO_ERR_HTTP_CONNECTION_CLOSED  (AIIO_ERR_HTTP_BASE + 8)     /*!< Read FIN from peer and the connection closed */

/**
 * @brief      Start a HTTP session
 *             This function must be the first function to call,
 *             and it returns a aiio_http_client_handle_t that you must use as input to other functions in the interface.
 *             This call MUST have a corresponding call to aiio_http_client_cleanup when the operation is complete.
 *
 * @param[in]  config   The configurations, see `http_client_config_t`
 *
 * @return
 *     - `aiio_http_client_handle_t`
 *     - NULL if any errors
 */
aiio_http_client_handle_t aiio_http_client_init(const aiio_http_client_config_t *config);

/**
 * @brief      Invoke this function after `aiio_http_client_init` and all the options calls are made, and will perform the
 *             transfer as described in the options. It must be called with the same aiio_http_client_handle_t as input as the aiio_http_client_init call returned.
 *             aiio_http_client_perform performs the entire request in either blocking or non-blocking manner. By default, the API performs request in a blocking manner and returns when done,
 ...
 *             the user may call this API multiple times unless request & response is complete or there is a failure. To enable non-blocking aiio_http_client_perform(), `is_async` member of aiio_http_client_config_t
 *             must be set while making a call to aiio_http_client_init() API.
 *             You can do any amount of calls to aiio_http_client_perform while using the same aiio_http_client_handle_t. The underlying connection may be kept open if the server allows it.
 *             If you intend to transfer more than one file, you are even encouraged to do so.
 *             aiio_http_client will then attempt to re-use the same connection for the following transfers, thus making the operations faster, less CPU intense and using less network resources.
 *             Just note that you will have to use `aiio_http_client_set_**` between the invokes to set options for the following aiio_http_client_perform.
 *
 ...
 *             If you want parallel transfers, you must use several aiio_http_client_handle_t.
 *             This function include `aiio_http_client_open` -> `aiio_http_client_write` -> `aiio_http_client_fetch_headers` -> `aiio_http_client_read` (and option) `aiio_http_client_close`.
 *
 * @param      client  The aiio_http_client handle
 *
 * @return
 ...
 *
 * @param[in]  client  The aiio_http_client handle
...
/**
 * @brief      Set post data, this function must be called before `aiio_http_client_perform`.
...
/**
 * @brief      Set http request header, this function must be called after aiio_http_client_init and before any
 *             perform function
 *
 * @param[in]  client  The aiio_http_client handle
 ...
 *             This function must be called after `aiio_http_client_init`.
 *
 * @param[in]  client  The aiio_http_client handle
 * @param[in]  key     The header key
 ...
 *
 * @param[in]  client    The aiio_http_client handle
 * @param[in]  auth_type The aiio_http_client auth type
 *
 * @return
 *     - AIIO_OK
 *     - AIIO_ERR_INVALID_ARG
...
/**
 * @brief     This function will write data to the HTTP connection previously opened by aiio_http_client_open()
 *
 * @param[in]  client  The aiio_http_client handle
 * @param      buffer  The buffer
 * @param[in]  len     This value must not be larger than the write_len parameter provided to aiio_http_client_open()
 *
 * @return
 *     - (-1) if any errors
...
/**
 * @brief      This function need to call after aiio_http_client_open, it will read from http stream, process all receive headers
 *
 * @param[in]  client  The aiio_http_client handle
 *
 * @return
 *     - (0) if stream doesn't contain content-length header, or chunked encoding (checked by `aiio_http_client_is_chunked` response)
 *     - (-1: AIIO_FAIL) if any errors
...
/**
 * @brief      Get http response status code, the valid value if this function invoke after `aiio_http_client_perform`
 *
 * @param[in]  client  The aiio_http_client handle
 *
 * @return     Status code
 ...
 *             the valid value if this function invoke after `aiio_http_client_perform`
 *
 * @param[in]  client  The aiio_http_client handle
 *
 * @return
 ...
 *             It is the opposite of the aiio_http_client_init function and must be called with the same handle as input that a aiio_http_client_init call returned.
 *             This might close all connections this handle has used and possibly has kept open until now.
 *             Don't call this function if you intend to transfer more files, re-using handles is a key to good performance with aiio_http_client.
 *
 * @param[in]  client  The aiio_http_client handle
 *
 * @return
 ...
 *             This is a helper API which internally calls `aiio_http_client_read` multiple times till the end of data is reached or till the buffer gets full.
 *
 * @param[in]  client   The aiio_http_client handle
 * @param      buffer   The buffer
 ...
 *              As no additional user-supplied buffer is required, this may be preferrable to `aiio_http_client_read_response` in situations where the content of the response may be ignored.
 *
 * @param[in]  client  The aiio_http_client handle
 * @param      len     Length of data discarded
 *
 * @return
 *     - AIIO_OK                 If successful, len will have discarded length
 *     - AIIO_FAIL               If failed to read response
 *     - AIIO_ERR_INVALID_ARG    If the client is NULL
 */
 ...
 *
 * @return
 *     - AIIO_OK                 If successful, len will have length of current chunk
 *     - AIIO_FAIL               If the server is not a chunked server
 *     - AIIO_ERR_INVALID_ARG    If the client or len are NULL
 */
