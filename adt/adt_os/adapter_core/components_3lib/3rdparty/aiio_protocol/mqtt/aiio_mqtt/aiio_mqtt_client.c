...
#ifdef AIIO_EVENT_ANY_ID
_Static_assert(MQTT_EVENT_ANY == AIIO_EVENT_ANY_ID, "mqtt-client event enum does not match the global EVENT_ANY_ID");
#endif
...
static aiio_err_t aiio_mqtt_dispatch_event(aiio_mqtt_client_handle_t client);
static aiio_err_t aiio_mqtt_dispatch_event_with_msgid(aiio_mqtt_client_handle_t client);
static void aiio_mqtt_destroy_config(aiio_mqtt_client_handle_t client);
...
#if MQTT_ENABLE_SSL
enum aiio_mqtt_ssl_cert_key_api {
...
static aiio_err_t aiio_mqtt_set_ssl_transport_properties(aiio_transport_list_handle_t transport_list, mqtt_config_storage_t *cfg)
{
    ...
    AIIO_OK_CHECK(aiio_mqtt_set_cert_key_data(ssl, MQTT_SSL_DATA_API_CLIENT_CERT, cfg->clientcert_buf, cfg->clientcert_bytes),
                 goto aiio_mqtt_set_transport_failed);
    AIIO_OK_CHECK(aiio_mqtt_set_cert_key_data(ssl, MQTT_SSL_DATA_API_CLIENT_KEY, cfg->clientkey_buf, cfg->clientkey_bytes),
                 goto aiio_mqtt_set_transport_failed);
    ...
    if (cfg->clientkey_password && cfg->clientkey_password_len) {
        ...
        aiio_transport_ssl_set_client_key_password(ssl,
...
aiio_mqtt_set_transport_failed:
    return AIIO_FAIL;
}
...
aiio_err_t aiio_mqtt_set_config(aiio_mqtt_client_handle_t client, const aiio_mqtt_client_config_t *config)
{
    ...
    err = AIIO_ERR_NO_MEM;
    AIIO_MEM_CHECK(set_if_config(config->host, &client->config->host), goto _mqtt_set_config_failed);
    AIIO_MEM_CHECK(set_if_config(config->path, &client->config->path), goto _mqtt_set_config_failed);
    AIIO_MEM_CHECK(set_if_config(config->username, &client->connect_info.username), goto _mqtt_set_config_failed);
    AIIO_MEM_CHECK(set_if_config(config->password, &client->connect_info.password), goto _mqtt_set_config_failed);
    ...
    AIIO_MEM_CHECK(set_if_config(config->lwt_topic, &client->connect_info.will_topic), goto _mqtt_set_config_failed);
    ...
    if (config->alpn_protos) {
        ...
        AIIO_MEM_CHECK(client->config->alpn_protos, goto _mqtt_set_config_failed);

        for (int i = 0; i < client->config->num_alpn_protos; i++) {
            client->config->alpn_protos[i] = strdup(config->alpn_protos[i]);
            AIIO_MEM_CHECK(client->config->alpn_protos[i], goto _mqtt_set_config_failed);
        }
    }
    ...
    if (config->clientkey_password && config->clientkey_password_len) {
        ...
        AIIO_MEM_CHECK(client->config->clientkey_password, goto _mqtt_set_config_failed);
    ...
    if (config->transport) {
        ...
        if (config->transport == MQTT_TRANSPORT_OVER_TCP) {
            ...
            AIIO_MEM_CHECK(client->config->scheme, goto _mqtt_set_config_failed);
        }
#if MQTT_ENABLE_WS
    ...
    if (config->uri) {
        if (aiio_mqtt_client_set_uri(client, client->config->uri) != AIIO_OK) {
            err = AIIO_FAIL;
            goto _mqtt_set_config_failed;
        }
    }
    aiio_err_t config_has_conflict = aiio_mqtt_check_cfg_conflict(client->config, config);

    MQTT_API_UNLOCK(client);

    return config_has_conflict;
_mqtt_set_config_failed:
    aiio_mqtt_destroy_config(client);
    MQTT_API_UNLOCK(client);
    return err;
}
...
static void aiio_mqtt_destroy_config(aiio_mqtt_client_handle_t client)
{
    ...
    if (client->config->event_loop_handle) {
        aiio_event_loop_delete(client->config->event_loop_handle);
    }
#endif
...
static inline aiio_err_t aiio_mqtt_write(aiio_mqtt_client_handle_t client)
{
...
static aiio_err_t aiio_mqtt_connect(aiio_mqtt_client_handle_t client, int timeout_ms)
{
    ...
    if (aiio_mqtt_write(client) != AIIO_OK) {
        return AIIO_FAIL;
    }
    ...
    client->event.error_handle->aiio_tls_stack_err = 0;
    client->event.error_handle->aiio_tls_last_aiio_err = 0;
    client->event.error_handle->aiio_tls_cert_verify_flags = 0;
...
{
    MQTT_API_LOCK(client);
    aiio_transport_close(client->transport);
...
static bool create_client_data(aiio_mqtt_client_handle_t client)
{
    ...
    AIIO_MEM_CHECK(client->transport_list, return false);

    aiio_transport_handle_t tcp = aiio_transport_tcp_init();
    AIIO_MEM_CHECK(tcp, return false);
    aiio_transport_set_default_port(tcp, MQTT_TCP_DEFAULT_PORT);
    aiio_transport_list_add(client->transport_list, tcp, "mqtt");

#if MQTT_ENABLE_WS
    aiio_transport_handle_t ws = aiio_transport_ws_init(tcp);
    AIIO_MEM_CHECK(ws, return false);
    aiio_transport_set_default_port(ws, MQTT_WS_DEFAULT_PORT);
#ifdef MQTT_SUPPORTED_FEATURE_WS_SUBPROTOCOL
    aiio_transport_ws_set_subprotocol(ws, "mqtt");
#endif
    aiio_transport_list_add(client->transport_list, ws, "ws");
#endif

#if MQTT_ENABLE_SSL
    aiio_transport_handle_t ssl = aiio_transport_ssl_init();
    AIIO_MEM_CHECK(ssl, return false);
    aiio_transport_set_default_port(ssl, MQTT_SSL_DEFAULT_PORT);

    aiio_transport_list_add(client->transport_list, ssl, "mqtts");
#endif

#if MQTT_ENABLE_WSS
    aiio_transport_handle_t wss = aiio_transport_ws_init(ssl);
    AIIO_MEM_CHECK(wss, return false);
#ifdef MQTT_SUPPORTED_FEATURE_WS_SUBPROTOCOL
    aiio_transport_ws_set_subprotocol(wss, "mqtt");
#endif
    aiio_transport_set_default_port(wss, MQTT_WSS_DEFAULT_PORT);
    aiio_transport_list_add(client->transport_list, wss, "wss");
#endif
    AIIO_MEM_CHECK(client->transport_list, return false);
    return true;
}
...
aiio_mqtt_client_handle_t aiio_mqtt_client_init(const aiio_mqtt_client_config_t *config)
{
    ...
    if (aiio_mqtt_set_config(client, config) != AIIO_OK) {
        goto _mqtt_init_failed;
    }
#ifdef MQTT_SUPPORTED_FEATURE_EVENT_LOOP
    aiio_event_loop_args_t no_task_loop = {
        .queue_size = 1,
        .task_name = NULL,
    };
    aiio_event_loop_create(&no_task_loop, &client->config->event_loop_handle);
#endif
    ...
    AIIO_MEM_CHECK(client->outbox, goto _mqtt_init_failed);
    client->status_bits = xEventGroupCreate();
    AIIO_MEM_CHECK(client->status_bits, goto _mqtt_init_failed);
...
static char *create_string(const char *ptr, int len)
{
    ...
    ret = calloc(1, len + 1);
    AIIO_MEM_CHECK(ret, return NULL);
    memcpy(ret, ptr, len);
    return ret;
}
...
aiio_err_t aiio_mqtt_client_set_uri(aiio_mqtt_client_handle_t client, const char *uri)
{
    ...
    if (puri.field_data[UF_PATH].len || puri.field_data[UF_QUERY].len) {
        ...
        AIIO_MEM_CHECK(client->config->path, {
            MQTT_API_UNLOCK(client);
            return AIIO_ERR_NO_MEM;
        });
    }
...
#if MQTT_ENABLE_WS
        aiio_transport_handle_t ws_trans = aiio_transport_list_get_transport(client->transport_list, "ws");
        if (ws_trans) {
            aiio_transport_ws_set_path(ws_trans, client->config->path);
        }
#endif
#if MQTT_ENABLE_WSS
        aiio_transport_handle_t wss_trans = aiio_transport_list_get_transport(client->transport_list, "wss");
        if (wss_trans) {
            aiio_transport_ws_set_path(wss_trans, client->config->path);
        }
#endif
    }
...
static aiio_err_t mqtt_write_data(aiio_mqtt_client_handle_t client)
{
    if (aiio_mqtt_write(client) != AIIO_OK) {
...
static aiio_err_t aiio_mqtt_dispatch_event_with_msgid(aiio_mqtt_client_handle_t client)
{
    ...
    return aiio_mqtt_dispatch_event(client);
}
...
post_data_event:
    ...
    aiio_mqtt_dispatch_event(client);
...
static aiio_err_t mqtt_process_receive(aiio_mqtt_client_handle_t client)
{
    uint8_t  msg_type;
    uint8_t  msg_qos;
    uint16_t msg_id;
...
static aiio_err_t mqtt_resend_queued(aiio_mqtt_client_handle_t client, outbox_item_handle_t item)
{
    // decode queued data
...
static void mqtt_delete_expired_messages(aiio_mqtt_client_handle_t client)
{
...
static void aiio_mqtt_task(void *pv)
{
    ...
    if (client->config->port == 0) {
        client->config->port = aiio_transport_get_default_port(client->transport);
    }
...
aiio_err_t aiio_mqtt_client_stop(aiio_mqtt_client_handle_t client)
{
    ...
    if (client->run) {
        ...
        if (client->state == MQTT_STATE_CONNECTED) {
            if (send_disconnect_msg(client) != AIIO_OK) {
                MQTT_API_UNLOCK(client);
                return AIIO_FAIL;
            }
        }
...
int aiio_mqtt_client_publish(aiio_mqtt_client_handle_t client, const char *topic, const char *data, int len, int qos, int retain)
{
    ...
    if (client->state != MQTT_STATE_CONNECTED) {
        aiio_log_i("Publishing skipped: client is not connected");
        MQTT_API_UNLOCK(client);
        return -1;
    }
#endif
...
static void aiio_mqtt_client_dispatch_transport_error(aiio_mqtt_client_handle_t client)
{
    ...
    client->event.error_handle->aiio_tls_last_aiio_err = aiio_tls_get_and_clear_last_error(aiio_transport_get_error_handle(client->transport),
            &client->event.error_handle->aiio_tls_stack_err,
            &client->event.error_handle->aiio_tls_cert_verify_flags);
#ifdef MQTT_SUPPORTED_FEATURE_TRANSPORT_SOCK_ERRNO_REPORTING
    client->event.error_handle->aiio_transport_sock_errno = aiio_transport_get_errno(client->transport);
#endif
#endif
    aiio_mqtt_dispatch_event_with_msgid(client);
}
