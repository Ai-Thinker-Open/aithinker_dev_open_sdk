...
struct tcp_connect_test_config {
    aiio_transport_handle_t transport_under_test;
...
static void localhost_listener(void *pvParameters)
{
    ...
    if (err != 0) {
        aiio_log_e( "Error occurred during listen: errno %d", errno);
        storage->tcp_listener_failed = true;
        goto failed;
    }
    ...
    if (storage->config.consume_sock_backlog) {
        ...
        if (storage->accepted_sock < 0) {
            aiio_log_e( "Unable to accept connection: errno %d", errno);
            goto failed;
        }
...
static void tcp_connect_task(void *pvParameters)
{
    ...
    int (*connect_fn)(aiio_transport_handle_t, const char *, int, int) =
            storage->config.connect_async ? aiio_transport_connect_async : aiio_transport_connect;
 ...
 */
void tcp_transport_test_connection_timeout(aiio_transport_handle_t transport_under_test)
{
 ...
 */
void tcp_transport_test_socket_options(aiio_transport_handle_t transport_under_test, bool async,
                                       const struct expected_sock_option *expected_opts, size_t sock_options_len)
{
    ...
    int sock = aiio_transport_get_socket(params.transport_under_test);
