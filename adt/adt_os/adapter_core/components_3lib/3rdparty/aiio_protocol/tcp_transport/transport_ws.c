...
static int ws_connect(aiio_transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    // See aiio_crypto_sha256() arg size
    unsigned char expected_server_sha256[32];
    // Size of base64 coded string see above
    unsigned char expected_server_key[45] = {0};
    ...
    aiio_crypto_sha256(expected_server_text, key_len, expected_server_sha256);
    aiio_crypto_base64_encode(expected_server_key, sizeof(expected_server_key),  &outlen, expected_server_sha256, sizeof(expected_server_sha256));
