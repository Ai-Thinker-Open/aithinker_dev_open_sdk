...
http_header_handle_t http_header_init(void)
{
    ...
    AIIO_RETURN_ON_FALSE(header, NULL, TAG, "Memory exhausted");
    STAILQ_INIT(header);
    return header;
}
...
static int http_header_new_item(http_header_handle_t header, const char *key, const char *value)
{
    ...
    AIIO_RETURN_ON_FALSE(item, AIIO_ERR_NO_MEM, TAG, "Memory exhausted");
    http_utils_assign_string(&item->key, key, -1);
    AIIO_GOTO_ON_FALSE(item->key, AIIO_ERR_NO_MEM, _header_new_item_exit, TAG, "Memory exhausted");
    http_utils_trim_whitespace(&item->key);
    http_utils_assign_string(&item->value, value, -1);
    AIIO_GOTO_ON_FALSE(item->value, AIIO_ERR_NO_MEM, _header_new_item_exit, TAG, "Memory exhausted");
...
int http_header_set_from_string(http_header_handle_t header, const char *key_value_data)
{
    ...
    AIIO_RETURN_ON_FALSE(p_str, AIIO_ERR_NO_MEM, TAG, "Memory exhausted");
    eq_ch = strchr(p_str, ':');
    if (eq_ch == NULL) {
        free(p_str);
...
int http_header_set_format(http_header_handle_t header, const char *key, const char *format, ...)
{
    ...
    va_end(argptr);
    AIIO_RETURN_ON_FALSE(buf, 0, TAG, "Memory exhausted");
    if (buf == NULL) {
        return 0;
    }
...
int http_header_generate_string(http_header_handle_t header, int index, char *buffer, int *buffer_len)
{
    ...
    STAILQ_FOREACH(item, header, next) {
        ...
        if (siz + 1 > *buffer_len - 2) {
            ...
            ret_idx = idx - 1;
            aiio_log_e( "Buffer length is small to fit all the headers");
            break;
        }
    }
