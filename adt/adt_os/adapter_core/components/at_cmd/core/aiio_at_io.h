...
/** @brief check AT uart param
 *
 *  @return
 *  @note
 *  @see
 */
CORE_API int32_t aiio_at_uart_param_check(void);
...
CORE_API void aiio_at_printf_mutex_lock(void);

CORE_API void aiio_at_printf_mutex_unlock(void);

CORE_API void aiio_at_printf_unlock(char *fmt, ...);

CORE_API void aiio_at_send_data_unlock(const uint8_t *data, uint32_t len);
#endif
