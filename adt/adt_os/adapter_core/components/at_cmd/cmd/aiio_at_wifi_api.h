...
#define AIIO_CONFIG_WIFI_RECONNECT_THREAD_SIZE  (1024*4)
#define AIIO_CONFIG_WIFI_RECONNECT_THREAD_PRIORITY  (2)
...
#if AIIO_CONFIG_IPV6    
    aiio_at_callbackfunc wifiGetIPV6Callback;
#endif    
...
typedef enum{
    ...
    AIIO_AT_WIFI_COUNTRY_MN=17, //Mongolia
    AIIO_AT_WIFI_COUNTRY_CL=19, //chile
...
//#if !AIIO_CONFIG_AT_SOCKET_DEL_SYNC
    AIIO_AT_CONNECT_STATUS_WAIT_TO_DEL=127, //socket wait to del
//#endif	//end of #if !AIIO_CONFIG_AT_SOCKET_DEL_SYNC
...
typedef enum {
    ...
    AIIO_AT_SECURITY_WPA_PSK,              //WPA
    AIIO_AT_SECURITY_WPA_Enterprise,   //WPA Enterprise
    AIIO_AT_SECURITY_WPA_TKIP_PSK,              //WPA TKIP
    AIIO_AT_SECURITY_WPA_TKIP_Enterprise,   //WPA TKIP Enterprise
    AIIO_AT_SECURITY_WPA_WPA2_PSK,         //WPA/WPA2
    ...
    AIIO_AT_SECURITY_WPA2_PSK,         //WPA2 PSK
    ...
    AIIO_AT_SECURITY_WPA3_AES_PSK,          //WPA3 AES
    AIIO_AT_SECURITY_WPA3_Enterprise,          //WPA3 Enterprise
...
typedef struct {
    ...
    aiio_at_enable_t auto_reconnect;    //Auto reconnect after disconnect or boot connect failure
...
void aiio_set_app_reconnect_flag(uint8_t flag);
void aiio_del_wifi_reconnect_thread(void);
CORE_API uint8_t aiio_at_wifi_init(void);
CORE_API void aiio_at_wifi_set_country_code(void);
...
CORE_API uint8_t aiio_at_wifi_connect(aiio_at_wifi_mode_t wifimode, aiio_at_sta_dhcpinfo_t *psta_dhcpinfo, char *ssid, char *passwd, uint8_t *pbssid, uint8_t onboot,aiio_wifi_eap_type eapType,char* staIdentity);
...
CORE_API uint8_t aiio_at_wifi_get_apclient_list(uint8_t *p_aplistcount,aiio_wifi_sta_basic_info_t *p_aplistarray);
