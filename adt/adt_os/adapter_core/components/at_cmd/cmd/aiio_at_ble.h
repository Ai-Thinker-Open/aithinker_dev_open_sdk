...
#define AT_BLE_ADV_DATARAW_SIZE 31
...
#define BLE_ADV_MAX             180
#define BLE_MIN_INTV            32
#define BLE_MAX_INTV            32
#define BLE_LATENCY             0
#define BLE_TIMEOUT             600
...
#define AT_BLE_PASSTHROUGH_SIZE (247 - 3)
...
typedef struct
{
    ...
    struct {
        uint16_t adv_length;
        uint16_t rsp_length;
        uint8_t adv_data[AT_BLE_ADV_DATARAW_SIZE];
        uint8_t rsp_data[AT_BLE_ADV_DATARAW_SIZE];
    } adv_raw_cfg;
...
void at_ble_io_state_set(void);     // 根据当前连接状态设置BLE IO的状态
