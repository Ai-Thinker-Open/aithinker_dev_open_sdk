...
typedef void (*aiio_airkiss_timeout_callback_t)(void);
...
/** @brief  Register the airkiss timeout callback.
 *  @param[in] cb       Timeout callback function.
 *  @return             Return the operation status. When the return value is AIIO_OK, the register is successful.
 *  @retval             AIIO_OK         register successful.
 *  @retval             AIIO_ERROR      register error.
 *  @note
 *  @see
 */
CORE_API int32_t aiio_airkiss_register_timeout_cb(aiio_airkiss_timeout_callback_t cb);
#endif
