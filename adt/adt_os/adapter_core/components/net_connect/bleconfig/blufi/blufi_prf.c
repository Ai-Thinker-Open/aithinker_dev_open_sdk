...
void btc_blufi_report_error(aiio_blufi_error_state_t state);

void btc_blufi_send_encap(uint8_t type, uint8_t *data, int32_t total_data_len)
{
    struct blufi_hdr *hdr = NULL;
    int32_t remain_len = total_data_len;
    uint16_t checksum;
    int32_t ret;

    if (blufi_env.is_connected == false) {
        aiio_log_e("blufi connection has been disconnected \r\n");
        return;
    }
        ...
        if (BLUFI_TYPE_IS_CTRL(hdr->type)) {
            ...
            if ((blufi_env.sec_mode & BLUFI_DATA_SEC_MODE_ENC_MASK)
                ...
                if (ret == hdr->data_len) { /* enc must be success and enc len must equal to plain len */
                    ...
                    aiio_log_e("%s encrypt error %d\r\n", __func__, ret);
                    btc_blufi_report_error(AIIO_BLUFI_ENCRYPT_ERROR);
                    free(hdr);
                    return;
                }
            }
...
inline void btc_blufi_cb_to_app(aiio_blufi_cb_event_t event, aiio_blufi_cb_param_t *param)
{
    aiio_blufi_event_cb_t btc_blufi_cb = (aiio_blufi_event_cb_t)btc_profile_cb_get();
    if (btc_blufi_cb)
    {
...
{
    aiio_blufi_callbacks_t *store_p = blufi_env.cbs;

    uint8_t rc;
    if (blufi_env.enabled)
    {
        aiio_log_e("BLUFI already initialized\r\n");
        return AIIO_BLUFI_ERROR;
    }
...
// {
//     if (!blufi_env.enabled) {
//         aiio_log_e("BLUFI already de-initialized\r\n");
//         return AIIO_BLUFI_ERROR;
//     }
...
void btc_blufi_send_notify(uint8_t *pkt, int32_t pkt_len)
{
   struct pkt_info pkts;
   pkts.pkt = pkt;
   pkts.pkt_len = pkt_len;
   aiio_blufi_send_notify(&pkts);
}
...
void btc_blufi_report_error(aiio_blufi_error_state_t state)
{
    ...
    msg.act = AIIO_BLUFI_EVENT_REPORT_ERROR;
...
void btc_blufi_recv_handler(uint8_t *data, int32_t len)
{
    ...
    if (hdr->seq != blufi_env.recv_seq) {
        aiio_log_e("%s seq %d is not expect %d\r\n", __func__, hdr->seq, blufi_env.recv_seq + 1);
        btc_blufi_report_error(AIIO_BLUFI_SEQUENCE_ERROR);
        return;
    }
    ...
    if (BLUFI_FC_IS_ENC(hdr->fc)
        ...
        if (ret != hdr->data_len) { /* enc must be success and enc len must equal to plain len */
            aiio_log_e("%s decrypt error %d\r\n", __func__, ret);
            btc_blufi_report_error(AIIO_BLUFI_DECRYPT_ERROR);
            return;
        }
    }
    ...
    if (BLUFI_FC_IS_CHECK(hdr->fc)
        ...
        if (checksum != checksum_pkt) {
            aiio_log_e("%s checksum error %04x, pkt %04x\r\n", __func__, checksum, checksum_pkt);
            btc_blufi_report_error(AIIO_BLUFI_CHECKSUM_ERROR);
            return;
        }
    }
    ...
    if (BLUFI_FC_IS_FRAG(hdr->fc)) {
        if (blufi_env.offset == 0) {
            ...
            if (blufi_env.aggr_buf == NULL) {
                aiio_log_e("%s no mem, len %d\r\n", __func__, blufi_env.total_len);
                btc_blufi_report_error(AIIO_BLUFI_DH_MALLOC_ERROR);
                return;
            }
        }
        if (blufi_env.offset + hdr->data_len  - 2 <= blufi_env.total_len){
            ...
            aiio_log_e("%s payload is longer than packet length, len %d \r\n", __func__, blufi_env.total_len);
            btc_blufi_report_error(AIIO_BLUFI_DATA_FORMAT_ERROR);
            return;
        }
...
{
    aiio_blufi_cb_param_t *dst = (aiio_blufi_cb_param_t *) p_dest;
    aiio_blufi_cb_param_t *src = (aiio_blufi_cb_param_t *) p_src;

    switch (msg->act) {
    case AIIO_BLUFI_EVENT_RECV_STA_SSID:
        dst->sta_ssid.ssid = malloc(src->sta_ssid.ssid_len);
        if (dst->sta_ssid.ssid == NULL) {
            aiio_log_e("[BLUFI] %s %d no mem\r\n", __func__, msg->act);
        }
...
{
    aiio_blufi_cb_param_t *param = (aiio_blufi_cb_param_t *)msg->arg;
...
void btc_blufi_cb_handler(btc_msg_t *msg)
{
    ...
    {
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_DEINIT_FINISH, param);
        break;
    }
    case AIIO_BLUFI_EVENT_BLE_CONNECT:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_BLE_CONNECT, param);
        break;
    case AIIO_BLUFI_EVENT_BLE_DISCONNECT:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_BLE_DISCONNECT, param);
        break;
    case AIIO_BLUFI_EVENT_SET_WIFI_OPMODE:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_SET_WIFI_OPMODE, param);
        break;
    case AIIO_BLUFI_EVENT_REQ_CONNECT_TO_AP:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_REQ_CONNECT_TO_AP, NULL);
        break;
    case AIIO_BLUFI_EVENT_REQ_DISCONNECT_FROM_AP:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_REQ_DISCONNECT_FROM_AP, NULL);
        break;
    case AIIO_BLUFI_EVENT_GET_WIFI_STATUS:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_GET_WIFI_STATUS, NULL);
        break;
    case AIIO_BLUFI_EVENT_GET_WIFI_LIST:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_GET_WIFI_LIST, NULL);
        break;
    case AIIO_BLUFI_EVENT_DEAUTHENTICATE_STA:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_DEAUTHENTICATE_STA, NULL);
        break;
    case AIIO_BLUFI_EVENT_RECV_STA_BSSID:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_STA_BSSID, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_STA_SSID:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_STA_SSID, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_STA_PASSWD:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_STA_PASSWD, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SOFTAP_SSID:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SOFTAP_SSID, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SOFTAP_PASSWD:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SOFTAP_PASSWD, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SOFTAP_MAX_CONN_NUM:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SOFTAP_MAX_CONN_NUM, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SOFTAP_AUTH_MODE:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SOFTAP_AUTH_MODE, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SOFTAP_CHANNEL:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SOFTAP_CHANNEL, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_USERNAME:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_USERNAME, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_CA_CERT:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_CA_CERT, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_CLIENT_CERT:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_CLIENT_CERT, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SERVER_CERT:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SERVER_CERT, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_CLIENT_PRIV_KEY:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_CLIENT_PRIV_KEY, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SERVER_PRIV_KEY:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SERVER_PRIV_KEY, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_SLAVE_DISCONNECT_BLE:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_SLAVE_DISCONNECT_BLE, param);
        break;
    case AIIO_BLUFI_EVENT_REPORT_ERROR:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_REPORT_ERROR, param);
        break;
    case AIIO_BLUFI_EVENT_RECV_CUSTOM_DATA:
        btc_blufi_cb_to_app(AIIO_BLUFI_EVENT_RECV_CUSTOM_DATA, param);
        break;
    default:
        aiio_log_e("[BLUFI]%s UNKNOWN %d\n", __func__, msg->act);
        break;
    }
...
void btc_blufi_call_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    case BTC_BLUFI_ACT_SEND_CFG_REPORT: {
        aiio_blufi_extra_info_t *src_info = src->wifi_conn_report.extra_info;
        ...
        dst->wifi_conn_report.extra_info = malloc(sizeof(aiio_blufi_extra_info_t));
        memset(dst->wifi_conn_report.extra_info,0,sizeof(aiio_blufi_extra_info_t));
    ...
    case BTC_BLUFI_ACT_SEND_WIFI_LIST:{
        aiio_blufi_ap_record_t *list = src->wifi_list.list;
        src->wifi_list.list = NULL;
        if (list == NULL || src->wifi_list.apCount <= 0) {
            break;
        }
        dst->wifi_list.list = (aiio_blufi_ap_record_t *)malloc(sizeof(aiio_blufi_ap_record_t) * src->wifi_list.apCount);
        if (dst->wifi_list.list == NULL) {
            break;
        }
        memcpy(dst->wifi_list.list, list, sizeof(aiio_blufi_ap_record_t) * src->wifi_list.apCount);
        break;
    }
    case BTC_BLUFI_ACT_SEND_CUSTOM_DATA:{
        uint8_t *data = src->custom_data.data;
        if(data == NULL) {
            aiio_log_e("custom data is NULL\r\n");
            break;
        }
        dst->custom_data.data = malloc(src->custom_data.data_len);
        if(dst->custom_data.data == NULL) {
            aiio_log_e("custom data malloc error\r\n");
            break;
        }
        memcpy(dst->custom_data.data, src->custom_data.data, src->custom_data.data_len);
        break;
...
void btc_blufi_call_deep_free(btc_msg_t *msg)
{
    ...
    case BTC_BLUFI_ACT_SEND_CFG_REPORT:
    {
        aiio_blufi_extra_info_t *info = (aiio_blufi_extra_info_t *)arg->wifi_conn_report.extra_info;
    ...
    case BTC_BLUFI_ACT_SEND_WIFI_LIST:
    {
        aiio_blufi_ap_record_t *list = (aiio_blufi_ap_record_t *)arg->wifi_list.list;
        if (list)
        {
            free(list);
        }
...
static void btc_blufi_send_error_info(uint8_t state)
{
    ...
    *p++ = state;
    if (p - data > data_len) {
        aiio_log_e("%s len error %d %d\r\n", __func__, (int32_t)(p - data), data_len);
    }
...
static void btc_blufi_send_custom_data(uint8_t *value, uint32_t value_len)
{
    if(value == NULL || value_len == 0) {
        aiio_log_e("%s value or value len error\r\n", __func__);
        return;
    }
    uint8_t *data = malloc(value_len);
    if (data == NULL) {
        aiio_log_e("%s mem malloc error\r\n", __func__);
        return;
    }
...
static void btc_blufi_wifi_conn_report(uint8_t opmode, uint8_t sta_conn_state, uint8_t softap_conn_num, aiio_blufi_extra_info_t *info, int32_t info_len)
{
    uint8_t type;
    uint8_t *data;
    int32_t data_len;
...
void btc_blufi_set_callbacks(aiio_blufi_callbacks_t *callbacks)
{
