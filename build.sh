...
# Board variants: bl616cl-48/-32 share the bl616cl platform(build+adt); bl618d* is its own platform.
# Both bl616cl* and bl618d* reuse the bl616cl SDK source (os/bl616cl_sdk_lts). Other chips map to self.
chip_platform() { case "$1" in bl616cl | bl616cl-*) echo bl616cl ;; bl618d | bl618d-*) echo bl618d ;; *) echo "$1" ;; esac; }
chip_sdk()      { case "$1" in bl616cl | bl616cl-* | bl618d | bl618d-*) echo bl616cl ;; *) echo "$1" ;; esac; }
...
if [[ $# -le 2 ]] && [[ "$1" == "update" ]]; then
    ...
    # 去掉第一个参数 "update"，后续全部传给 update.sh
    shift
    if [[ $# -eq 0 ]]; then
...
if [[ $# = 1 ]] && [[ "$1" == "environment" ]]; then
    ...
    sudo apt-get install git wget flex \
        ...
        libc6-i386 make bc gawk python3-pip libncurses5-dev gcc binutils

    pip install fpvgcc
    sudo chmod 777 /opt
    mkdir /opt/rtk-toolchain
    cd ./os/rtl8711dx_sdk_lts/sdk/ameba-rtos/amebadplus_gcc_project
...
if [[ $# == 3 ]] && [[ "$3" == "clean" ]]; then
    ...
    ## sdk
    rtos_dir=$root_dir/os/$(chip_sdk "$chip")"_sdk_lts"

    ## application demo
    application_dir=$root_dir/application/$application

    ## sdk_make
    make_dir=$root_dir/build/$(chip_platform "$chip")"_make"
